# CHMM Paper, Proff-Prep Summary

Last updated 2026-05-05 after the review-response sweep on top of the 2026-05-02 arXiv-prep baseline and the 2026-05-03/04 follow-on trim. The 2026-05-02 pass covered: contribution recasting, related-work expansion (signature/diffusion + Fearnhead-Liu), three appendix trims, 12-site reviewer-language strip, three orphan-stub deletions, pipeline-schematic body/appendix re-alignment, intro `5pp` and QuantGAN-footnote tightening, and five model-repo runner archivals. The 2026-05-03/04 sweep (commits `493fcd7`, `89d9c52`, `675247f`, `f88551f`, `e1aa093`) further trimmed the supplementary block (`supplementary.tex` shed ~189 lines, plus targeted line-edits across `sensitivity_appendix.tex`, `algorithms_appendix.tex`, `baselines_appendix.tex`, `cross_asset_appendix.tex`, `var_backtest.tex`, and `theory.tex`) and renamed the appendix block "Pipeline Schematic" → "Pipeline Summary". The 2026-05-05 review-response sweep (commits `7e95bdb`, `57939b7`, `88758eb`, `4cffee4`) executes Wave 1 (text-only edits) and folds in the runner results from review items 2A, 3, 6, 9, 11 documented in `REVIEW_RESPONSE_PLAN.md`. The PDF now lands at **66 pages** (body 1-15 exact, admin block + references 15-20, supplementary 21-65), clean compile, zero undefined references.
...
The paper trains a continuous Hidden Markov Model (CHMM) by Baum-Welch as a **synthetic-data generator for daily US-equity returns**. "Synthetic-data generator" means: given an observed return series, fit a model and then draw long simulated paths that look statistically like the original (same heavy tails, same volatility-clustering signature, same VaR breach behavior), so the paths can be used downstream in stress tests, regulatory backtests, copula composition, and privacy-preserving data sharing. The headline empirical claim is that the CHMM reproduces the three symmetric Cont-2001 stylized facts (heavy-tailed marginals, near-zero linear ACF, slow absolute-return ACF) at moderate state counts, on par with GARCH(1,1) on the volatility-clustering axis where simple HMMs are usually dismissed. The headline theoretical claim is that the textbook bilinear-ACF identity for HMMs implies a **rank constraint on the deflated transition matrix that is non-binding at the cross-ticker median for K ≥ 3 on equity-return data**, with the bound still doing visible work on the left tail (minimum dominant-mode share 32.6% on NEM); this mathematically explains why the famous Rydén-Teräsvirta-Åsbrink (1998) low-K HMM failure was a low-K artefact rather than an architectural ceiling, and the median-vs-tail framing is deliberate to prevent over-reading. The headline risk-management contribution is a **regime-conditional Value-at-Risk** that propagates the one-step-ahead state forecast through the predictive mixture and passes the Christoffersen joint conditional-coverage test at α = 0.05 across emission families on the headline OoS window and on 19/24 walk-forward rows; two non-state-space conditional-VaR contenders (filtered Hull-White-style historical simulation and Engle-Manganelli SAV CAViaR) match the CHMM on Christoffersen-cc at α = 0.05 but the strict-tail Engle-Manganelli DQ test rejects every contender row except CHMM-N at K\* = 3, so the CHMM construction has approximate parity at α = 0.05 with strict-tail superiority at K\* = 3. Scope: daily US equities; symmetric stylized facts only (leverage and gain-loss asymmetry are out-of-scope under symmetric per-state emissions); a non-equity stress test on GLD is included as a documented negative result (static-fit OoS KS collapses to 0%; quarterly refit lifts to 2.5%, still architecturally mis-specified).

---

## 1. arXiv-Readiness Verdict

**Status: ready** after the 2026-05-02 arXiv-prep trim, the 2026-05-03/04 follow-on sweep, and the 2026-05-05 review-response sweep; remaining cleanup in Section 12 is companion-repo only.

Build evidence: **66-page PDF** (body 15 pages exact, admin block + references on 15-20, supplementary 21-65; down from 112 over four reductions: 112 → 84 → 76 → 63 → 66, with the last leg slightly inflating the page count to fold in the new Wave 1 / Wave 2 deliverables — vendor-stitch appendix, λ-CV K=3 appendix, stationarity-scope table, two new VaR-contender rows, sector quarterly-refit-at-K=3 paragraph, and the variant decision guide moved into the supplementary). Clean compile, zero undefined references or labels, zero `[CITE]/[TBD]/TODO` placeholders, all `\includegraphics` 1-to-1 mapped, headline tables intact, 1 theorem with full proof, 4 propositions, 2 assumptions. Abstract is **citation-free and arXiv-compliant**, with no em dashes anywhere in the paper prose (per the no-em-dash convention; double-hyphen `--` substitutes inside table cells). Reproducibility scaffold complete: deterministic seed root 20260420 with documented sub-seed rule, Julia ≥ 1.12 with pinned `Manifest.toml`, R 4.6 + `renv` lockfile for the MSGARCH reference, public companion repos linked in the conclusion (`https://github.com/altashly1/CHMM-paper`, `https://github.com/altashly1/CHMM-Model`).

Section-file LOC at 2026-05-05 (`wc -l sections/*.tex`): `sensitivity_appendix.tex` 505, `algorithms_appendix.tex` 304, `supplementary.tex` 290, `baselines_appendix.tex` 256, `cross_asset_appendix.tex` 197, `metrics_appendix.tex` 141, `results.tex` 133, `estimation.tex` 54, `var_backtest.tex` 54, `model.tex` 41, `discussion.tex` 28, `related_work.tex` 17, `walkforward_body_table.tex` 14, `theory.tex` 13, `introduction.tex` 13, `conclusion.tex` 4. Total LaTeX 2,064 lines.
...
- `supplementary.tex` lost ~189 lines (commit `f88551f`); the Pipeline-B copula appendix block, the per-ticker $\hat\lambda^\star$ sweep prose, and several auxiliary tables were tightened or moved into the existing `cross_asset_appendix.tex` / `sensitivity_appendix.tex` blocks rather than duplicated in `supplementary.tex`.
...
- Renamed `\subsubsection{Pipeline Schematic}` to `\subsubsection{Pipeline Summary}` in `algorithms_appendix.tex` (commit `e1aa093`); the body-side reference in `model.tex` reads "step-by-step summary in Appendix~\ref{sec:supp_pipeline_summary}".
- All body-referenced labels survive; PDF compile clean.

What the 2026-05-05 review-response sweep added (deltas vs the 2026-05-04 baseline):
- **Title trimmed**: dropped the "Spectral Rank" subtitle. Current title (`paper.tex:122`): "Continuous Hidden Markov Models for Equity Returns: Heavy-Tail Emission Families and Regime-Conditional Value-at-Risk" (Item 19).
- **Theory promoted to its own top-level section**: `\section{Spectral Mechanism}` with `\label{sec:spectral}` (was a subsection inside Methods); the legacy `sec:theory` label is retained inside `theory.tex` so existing back-refs still resolve (Item 20).
- **Abstract rewritten**: citation-free, no em dashes, leads with the walk-forward median (62.1% CHMM-N OoS KS at $K^\star = 3$), explicitly qualifies the Christoffersen-cc pass to $\alpha = 0.05$, flags the Engle-Manganelli DQ rejection of $K = 18$ at $\alpha = 0.01$ as power-driven, and adds the median-vs-tail spectral-rank framing (Items 1, 7, 10).
- **Variant decision guide moved**: was a body table in `discussion.tex`, now lives at `sec:variant_decision_guide` inside `supplementary.tex`. Discussion compresses to four short paragraphs.
- **TikZ architecture figure moved**: was in body `model.tex`, now `Figure~\ref{fig:chmm_architecture}` inside `algorithms_appendix.tex` under `sec:supp_algorithms`; body sentence at `model.tex:21` now reads "architecture diagram in Appendix~\ref{sec:supp_algorithms}, Figure~\ref{fig:chmm_architecture}".
- **Two new VaR contenders in Table 5** (`var_backtest.tex` `tab:cond_var`): filtered Hull-White-style historical simulation and Engle-Manganelli (2004) SAV CAViaR, scored on the same Christoffersen / DQ harness. New body paragraph at `var_backtest.tex:51-52` reframes the lead as approximate parity at $\alpha = 0.05$ with strict-tail superiority at $K^\star = 3$ (Item 6).
- **Vendor-stitch sanity check**: new sentence in `model.tex:4` ("zero VWAP and return differential and Kolmogorov-Smirnov $p = 1.00$ on the 323-day overlap"), backed by new appendix `sec:vendor_stitch` (Item 9).
- **λ-CV at $K = 3$**: new appendix `sec:lambda_cv_pre2020` (within `sensitivity_appendix.tex`); held-out CV at $K = 3$ on the strictly pre-2020 slice selects $\lambda^\star = 50$ (vs the body's $\lambda = 20$ at $K = 18$). The body retains $\lambda = 20$ at $K^\star = 3$ as a sensitivity reference; the headline heavy-tail recipe at $K^\star = 3$ is now the **shared-$\nu$ Student-$t$ ablation** (no penalty hyperparameter), promoted in the variant decision guide (Items 3, 14).
- **Stationarity-scope table** (`tab:stationarity_scope`) added in `discussion.tex`: consolidates the four orthogonal stress sources (cross-ticker, cross-decade, GLD, walk-forward W2/W4) with their static-fit OoS KS and periodic-refit mitigation (Item 4).
- **Walk-forward median promoted**: 62.1% at $K^\star = 3$ now leads in both abstract and intro; the 80.5% single-window number is a parenthetical (Item 10).
- **Sector quarterly-refit at $K^\star = 3$** documented in `results.tex:78`: median OoS KS lifts $69.1\% \to 84.7\%$, failures $11/30 \to 8/30$ (alongside the existing $K = 18$ reference at $83.0\%$, $7/30$, with monthly cadence interpolating to $86.7\%$, $5/30$) (Item 2A leg 2).
- **GLD periodic refit** documented in `cross_asset_appendix.tex`: OoS KS $0\% \to 2.5\%$ at $K = 3$ quarterly refit; architecturally mis-specified for non-equity (Item 11).
- **Cross-asset Table 4 refreshed at $K^\star = 3$** (`tab:cross_asset`): off-diagonal MAE essentially $K$-robust ($0.027$ IS / $0.209$ OoS at both $K = 3$ and $K = 18$); per-asset KS sits lower at $K^\star = 3$ but the cross-asset task is dominated by the dependence layer (Item 2A leg 1).
- **HSMM-Gamma comparison**: new prose in `results.tex:68` documenting that a discrete-Gamma-sojourn HSMM at $K = 18$ closes the ACF gap to ACF-MAE $0.0462$, so the CHMM-vs-HSMM trade-off is not strictly that CHMM wins on ACF (Item 16).
- **Table 1 collapsed**: CRPS column moved into a tie-break sentence in the post-table prose; bolding policy switched to "panel-best within block" (block = i.i.d. baselines / GARCH family / MS-GARCH / CHMM body / co-headline). Removes the visual hijack by the bootstrap and GARCH-t (Items 13, 15).
- **Author Contributions** (`conclusion.tex`): C.J. credited specifically with "ECM-derivation methodology review and validation of the cross-decade and cross-ticker pipelines"; CoI / Contributions / Data Availability collapsed into one paragraph (Item 17).
- **MS-GARCH narration** reframed: `results.tex` now leads with the plug-in vs plug-in headline ("CHMM-N plug-in $89.7\%$ vs MS-GARCH plug-in $34$ to $36\%$"), with the Bayesian posterior-predictive row as a parenthetical (Item 5).
- **10 new bib entries** (`references.bib`): the citations needed for filtered-bootstrap / CAViaR (`barone1999var`, `engle2004caviar`), the new stationarity-scope citations (`pesarantimmermann2007window`, `cappe2011online`), and the existing rolling-window / regime-break references promoted from the appendix to body. Zero undefined or orphan keys.

**Companion repo runner scaffolding (Wave 2 + 3)**: four new runners landed (`runners/baselines/run_filtered_bootstrap_var.jl`, `runners/baselines/run_caviar_var.jl`, `runners/diagnostics/run_vendor_stitch_check.jl`, `runners/cross_asset/run_non_us_asset_quarterly_refit.jl`); three existing runners were patched to default to $K = 3$ with env-var override (`run_cross_asset_sim_copula.jl`, `run_lambda_cv_pre2020.jl`, `run_sector_panel_quarterly_refit.jl`). All four new runners have entries in `~/Desktop/Project-Repos/CHMM-Model/RUNNERS.md`. Items 8 and 12 (non-overlapping cross-asset, n=6 sector panel) are **already addressed by the existing appendix machinery** per the verification in `REVIEW_RESPONSE_PLAN.md`; no further runner work needed.
...
| **CHMM-t shared-$\nu$** | $2K + 1$ joint | Power-law, single $\nu$ | ECM (aggregate-$Q$ on $\nu$) | **4.68** | **Headline heavy-tail recipe; no penalty hyperparameter** |
| CHMM-t penalised | 3 ($\mu, \sigma, \nu$) | Power-law, $\nu$-dependent | ECM (golden-section on $\nu_k$) + $1/\nu_k$ shrinkage | 14.91 (at $\lambda = 20$, tuned at $K = 18$) | Sensitivity reference; $\lambda$-CV at $K = 3$ selects $\lambda^\star = 50$ (Appendix `sec:lambda_cv_pre2020`) |
...
Source: variant decision guide at [sections/supplementary.tex `sec:variant_decision_guide`](sections/supplementary.tex), Table `tab:variant_choice` (moved from `discussion.tex` in the 2026-05-05 review-response sweep).
...
- **What can**: any multi-state regime-switching model with a forward-filter and a per-state predictive density (CHMM, HSMM, MS-GARCH); also the two non-state-space conditional-VaR contenders added in the 2026-05-05 review-response sweep — filtered Hull-White-style historical simulation (Barone-Adesi et al. 1999) and Engle-Manganelli (2004) SAV CAViaR.
- **What cannot**: i.i.d. generators (no state filter), bootstrap (no parametric per-state density), unconditional GARCH (single conditional variance, single innovation distribution, no breach-clustering correction).
- This is the structural-use-case argument made in [sections/results.tex](sections/results.tex) — CHMM offers parametric controls and the state-filter forecasting machinery that the bootstrap cannot serve, even though the bootstrap beats CHMM on raw KS.

### 8.6 Non-state-space contenders (filtered bootstrap, CAViaR)
The 2026-05-05 review-response sweep added two non-state-space conditional-VaR baselines so the regime-conditional VaR claim has a fair non-CHMM contender. Both are scored on the same Christoffersen-cc / Engle-Manganelli DQ harness as the CHMM rows of `tab:cond_var`.
- **Filtered bootstrap** (Hull-White-style filtered historical simulation): GARCH(1,1)-$t$ fit on IS, residuals standardised, residuals bootstrapped to produce a forward-filtered conditional VaR per OoS day. Numbers: $\alpha = 0.05$ $p_{\text{cc}} = 0.43$, $p_{\text{DQ}} = 0.55$; $\alpha = 0.01$ $p_{\text{cc}} = 0.16$, $p_{\text{DQ}} = 0.04$ (rejects).
- **CAViaR (SAV)**: Engle-Manganelli (2004) symmetric absolute-value CAViaR, IS-trained, OoS-applied. Numbers: $\alpha = 0.05$ $p_{\text{cc}} = 0.56$, $p_{\text{DQ}} = 0.76$; $\alpha = 0.01$ $p_{\text{cc}} = 0.14$, $p_{\text{DQ}} = 0.01$ (rejects strongly).
- **Net story**: at $\alpha = 0.05$ the four rows (CHMM-N at $K^\star = 3$, CHMM-N at $K = 18$, filtered bootstrap, CAViaR) are statistically indistinguishable on Christoffersen-cc; at $\alpha = 0.01$ the DQ test rejects every row except CHMM-N at $K^\star = 3$ ($p_{\text{DQ}} = 0.06$). So the CHMM construction has approximate parity at $\alpha = 0.05$ with strict-tail superiority at $K^\star = 3$. CHMM also retains the structural advantages the contenders cannot serve (multi-asset copula composition under preserved per-asset marginals, parametric privacy, latent-state interpretability).

### 8.7 The α = 0.01 power caveat
...
- **Christoffersen-cc passes on the headline window** at the conventional $\alpha = 0.05$ (regime-conditional VaR, propagating one-step-ahead state forecasts through the predictive mixture). Six-fold rolling-origin walk-forward (5y train / 1y test): the conditional Christoffersen-cc passes on **19/24 rows**; the body OoS-KS distribution has median 62.1% (K=3) / 67.7% (K=18), with W2 (COVID) and W4 (2022 rate-hike onset) folds below 10% for every generator. Periodic refit (quarterly cadence) is the deployment recipe. The two non-state-space contenders added in the 2026-05-05 review-response sweep (filtered Hull-White-style historical simulation and Engle-Manganelli SAV CAViaR) match CHMM at $\alpha = 0.05$ on Christoffersen-cc but DQ rejects every contender at $\alpha = 0.01$ except CHMM-N at $K^\star = 3$.

- **Non-equity stress test (GLD) collapses under static fitting.** Reported as a negative result; it is the boundary of the headline claim. The 2026-05-05 GLD quarterly-refit run (Item 11) lifts OoS KS from 0% to 2.5%; the architecture is mis-specified for non-equity and periodic refit does not recover. The body sentence is on this in `cross_asset_appendix.tex` after the GLD Pipeline-A diagnostic.
...
- [x] **Follow-on trim sweep** (2026-05-03/04, commits `493fcd7`, `89d9c52`, `675247f`, `f88551f`, `e1aa093`): supplementary block shed ~189 lines, line-level trims across `sensitivity_appendix.tex` / `cross_asset_appendix.tex` / `theory.tex` / `var_backtest.tex` / `discussion.tex`, and renamed `\subsubsection{Pipeline Schematic}` → `\subsubsection{Pipeline Summary}` (label `sec:supp_pipeline_schematic` → `sec:supp_pipeline_summary`). PDF landed at 63 pages, clean compile, no undefined refs.
- [x] **Review-response sweep** (2026-05-05, commits `7e95bdb`, `57939b7`, `88758eb`, `4cffee4`): Wave 1 text-only edits (title trim, theory promoted to top-level section, abstract rewrite, CRPS column moved to prose, Table 1 within-block bolding, variant guide moved to supplementary, TikZ architecture moved to algorithms appendix, Author Contributions clarified) plus runner-result fold-in for review items 2A (cross-asset and sector quarterly-refit at $K^\star = 3$), 3 ($\lambda$-CV at $K^\star = 3$ giving $\lambda^\star = 50$), 6 (filtered-bootstrap and CAViaR contender rows in Table 5), 9 (vendor-stitch sanity check, KS $p = 1.00$), 11 (GLD quarterly refit $0\% \to 2.5\%$), 4 (stationarity-scope summary table in `discussion.tex`). Items 8 and 12 verified pre-existing in the appendix; no further runner work needed. PDF lands at **66 pages**, clean compile, zero undefined refs.
- [ ] User-facing: add the two repo links (CHMM-paper, CHMM-Model) and the seed-root statement (`20260420`) to the arXiv \emph{submission metadata page} at upload time. Both are already in the LaTeX (`conclusion.tex` and `model.tex` respectively).
...
- [ ] User-facing: pre-resubmit sign-off in `REVIEW_RESPONSE_PLAN.md` (cross-check abstract / intro contribution list / results headline / discussion variant guide / conclusion are consistent on $K^\star$, $\lambda$, and the Christoffersen-cc $\alpha$ qualifier). All 20 review items addressed in either text or numbers; final consistency pass recommended before PDF lockdown.
...
- `runners/baselines/run_filtered_bootstrap_var.jl`, `runners/baselines/run_caviar_var.jl`: non-state-space conditional-VaR contenders added in the 2026-05-05 review-response sweep; both feed the contender block of Table~`tab:cond_var` in `var_backtest.tex` (Item 6).
- `runners/diagnostics/run_vendor_stitch_check.jl`: Polygon vs Alpaca / IEX overlap diagnostic (Item 9); feeds the vendor-stitch appendix `sec:vendor_stitch` and the body-side sentence in `model.tex:4`.
- `runners/cross_asset/run_non_us_asset_quarterly_refit.jl`: GLD quarterly-refit follow-up (Item 11); feeds the `cross_asset_appendix.tex` paragraph and the Item 4 stationarity-scope summary table.
- `runners/robustness/run_lambda_cv_pre2020.jl` (defaulted to $K = 3$ with `LAMBDA_CV_K=18` env-var override) and `runners/robustness/run_sector_panel_quarterly_refit.jl` (defaulted to $K = 3$ with `SECTOR_PANEL_K=18` env-var override): feed the new appendix `sec:lambda_cv_pre2020` and the body sector quarterly-refit-at-$K = 3$ paragraph respectively (Items 3, 2A leg 2).
