...
The four emission families share a single forward-backward scaffold and quantile-based initialization; the M-step is the only architectural difference. \emph{CHMM-N}: $f_k = \mathcal{N}(\mu_k, \sigma_k^2)$. \emph{CHMM-t}: $f_k = t_{\nu_k}(\mu_k, \sigma_k)$ with per-state degrees of freedom $\nu_k \in [\nu_{\min}, \nu_{\max}]$. \emph{CHMM-L}: $f_k = \mathrm{Laplace}(\mu_k, \beta_k)$ with per-state Laplace scale denoted $\beta_k$ to avoid collision with the canonical Laplace-scale symbol $b$. \emph{CHMM-GED}: $f_k = \mathrm{GED}(\mu_k, \alpha_k, p_k)$ with per-state shape $p_k \in [p_{\min}, p_{\max}]$ that nests Gaussian ($p_k = 2$) and Laplace ($p_k = 1$). The architectural diagram is in Appendix~\ref{sec:supp_architecture}.
