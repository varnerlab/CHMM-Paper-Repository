...
For pairwise comparison we report a Diebold-Mariano test on the per-$t$ loss differential $d_t = \widehat{\text{CRPS}}_t^A - \widehat{\text{CRPS}}_t^B$ with a Newey-West HAC variance under a Bartlett kernel and bandwidth $h = \lfloor T_{\text{OoS}}^{1/3} \rfloor = 8$. Two-sided $p$-values are reported under the standard-normal null. The full pairwise table (CHMM-N, CHMM-t, CHMM-L each as the reference row against every other generator in Table~\ref{tab:model_comparison}) is in \texttt{results/track\_m11/CRPS\_DM.txt}; the body claim of Section~\ref{sec:model_comparison} is the headline reading of that table: CHMM ties or beats every benchmark on mean OoS CRPS, with Gaussian i.i.d.\ the only significantly worse comparator.
