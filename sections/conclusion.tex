...
The direct head-to-head against the discrete framework of \citet{alswaidan2026hybrid}, reproduced as the Discrete NJ and Discrete WJ rows of Table~\ref{tab:model_comparison} at the authoritative prior-paper hyperparameters ($K_{\text{disc}} = 90$, $\epsilon = 5 \times 10^{-5}$, $\lambda = 67$), shows that at this state resolution the discrete variants match the CHMM family on coarse KS/AD pass rates ($97.9$--$98.3\%$ IS KS, $96.5$--$98.3\%$ IS AD) but fall short on three structural dimensions: kurtosis ($3.5$ vs.\ observed $7.68$ and CHMM-t $14.57$), aggregational Gaussianity decay at $h = 10$ / $21$, and $5\%$ VaR Kupiec calibration (breach rate $1.7\%$ vs.\ target $5\%$; $\text{LR}_{\text{uc}} = 16.81$, $p < 10^{-3}$), each of which the continuous CHMM family clears.
The continuous-emissions claim is therefore a claim about tail structure and downstream VaR usability, not about distributional pass rates on the coarse KS/AD axis.
Relative to the Poisson-jump WJ variant, the continuous CHMM additionally removes two hyperparameters ($\epsilon$ for jump probability, $\lambda$ for mean jump duration) and the associated grid search, reducing model complexity.
