...
\subsection{Filter-Based Regime-Conditional VaR (Flagged Open Question)}
\label{sec:filter_var_note}

A natural further step is regime-conditional VaR using the forward-filter posterior. We considered this rule and found that it fails Kupiec out-of-sample for all three CHMM emission families at both $\alpha = 0.01$ and $\alpha = 0.05$ (breach rate $\approx 0.2\%$ vs.\ $1\%$ target and $\approx 1.2$--$1.4\%$ vs.\ $5\%$ target). The mechanism is a structural mixture-quantile inequality, the failure-rate breakdown, and the candidate remedies (concentration constraint on the filter posterior, state-conditional variance overlay, rolling re-estimation of $\hat\theta$) are deferred to Appendix~\ref{sec:supplementary}. We flag this as an honest open operational question rather than as a contribution of the present paper.
