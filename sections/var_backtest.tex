...
The Kupiec statistic only checks the unconditional breach rate. We complement it with the Christoffersen \cite{christoffersen1998evaluating} independence ($\text{LR}_{\text{ind}}$, $\sim\chi^2_1$) and joint conditional-coverage ($\text{LR}_{\text{cc}} = \text{LR}_{\text{uc}} + \text{LR}_{\text{ind}}$, $\sim\chi^2_2$) tests on the same pooled-archive breach series. The full panel is in Appendix~\ref{sec:christoffersen_supp}, Table~\ref{tab:christoffersen_var}; the headline reads as follows. Every \emph{unconditional} generator in the panel (i.i.d.\ bootstrap, GARCH(1,1), and all four CHMM variants) rejects independence on the OoS window at $\alpha = 0.05$ for both VaR levels. CHMM-N at $5\%$ has $\text{LR}_{\text{ind}} = 5.26$ ($p = 0.022$) and $\text{LR}_{\text{cc}} = 9.08$ ($p = 0.011$); GARCH(1,1) at $5\%$ has $\text{LR}_{\text{ind}} = 5.87$ ($p = 0.015$) and $\text{LR}_{\text{cc}} = 6.69$ ($p = 0.035$); the i.i.d.\ bootstrap rejects similarly. The independence rejection is driven by volatility-clustering in $R_{\text{oos}}$ itself, which any unconditional generator with a constant-across-$t$ VaR threshold is structurally unable to track: a regime-conditional VaR construction that propagates the latent-state forecast $\Prob(s_{t+1}\,|\,\text{history}_t)$ through the CHMM's emission-mixture is the natural extension and is deferred to a companion paper. The Christoffersen rejection is therefore a property of the unconditional VaR convention used here rather than a CHMM-specific failure, and the headline Kupiec conclusion (CHMM passes unconditional coverage at both levels on both windows) stands.
