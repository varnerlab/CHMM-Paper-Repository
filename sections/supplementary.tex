This appendix collects supplementary material referenced in the main body. Section~\ref{sec:supp_chmm_api} gives the public API and reproducer entry point of the companion Julia package \texttt{CHMM-Model.jl}. Section~\ref{sec:supp_algorithms} gives the EM forward-backward recursions and the algorithm pseudocode. Section~\ref{sec:supp_metrics} gives the precise validation metric definitions. Section~\ref{sec:supp_propositions} states the formal propositions cited inline in Section~\ref{sec:theory} (ECM monotonicity, identifiability, MLE consistency, rank-reordering marginal preservation, filter-VaR mixture-quantile bound). Section~\ref{sec:supp_sensitivity} gives the full state-resolution and multi-emission sensitivity panels, the $K=2$ replication, and the per-$K$ figures. Section~\ref{sec:supp_robustness} gives KS test-power calibration, the block-bootstrap KS recalibration, multi-seed Monte Carlo, walk-forward refit, and the block-bootstrap baseline; price-fan figures and an expanded GARCH panel are included for completeness. Section~\ref{sec:supp_cross_asset} gives the full cross-asset panel including SIM and Gaussian copula, with copula derivations and profile-MLE figure. Section~\ref{sec:supp_misc} collects the pre-OoS validation $K$-selection, the walk-forward summary, and the $\nu_k$-bracket / penalised-ECM diagnostics referenced from the discussion.
...
\begin{proposition}[Mixture-quantile lower bound, filter VaR]
\label{prop:filter_var}
Let $\boldsymbol\pi \in [0,1]^K$ be a probability vector, $F_1, \ldots, F_K$ per-state CDFs, $F = \sum_k \pi_k F_k$, and $k^* = \arg\min_k F_k^{-1}(\alpha)$ with $q^* = F_{k^*}^{-1}(\alpha)$. Then $F(q^*) \le \alpha$, with strict inequality whenever there exists $k \ne k^*$ with $\pi_k > 0$ and $F_k(q^*) < \alpha$. Consequently $F^{-1}(\alpha) \le \min_k F_k^{-1}(\alpha)$ under the same condition.
\end{proposition}
\noindent\emph{Proof sketch.} At $q^*$, $F_{k^*}(q^*) = \alpha$ and $F_k(q^*) \le \alpha$ for all $k$ by monotonicity, hence $F(q^*) = \sum_k \pi_k F_k(q^*) \le \alpha$, strict whenever some $F_k(q^*) < \alpha$ with $\pi_k > 0$. Monotonicity of $F$ lifts the CDF inequality to the quantile.
...
\subsection{Filter-Based Regime-Conditional VaR: Open Question}
\label{sec:supp_filter_var}

The body of Section~\ref{sec:filter_var_note} flags the failure of the regime-conditional filter-VaR rule and defers the operational detail here.

\paragraph{Construction.} Under the IS-fitted parameters $\hat\theta$, the forward filter posterior is $\pi_t(k) = \Prob(s_t = k \mid r_{1:t}, \hat\theta)$. The one-step-ahead predictive density at time $t$ is the mixture $f_{r_{t+1} \mid r_{1:t}}(x) = \sum_k \pi_t(k) \, f_k(x; \hat\theta)$, and the filter-based regime-conditional VaR at level $\alpha$ is the mixture quantile $\widehat{\text{VaR}}_t(\alpha) = F^{-1}_{\sum_k \pi_t(k) F_k}(\alpha)$.

\paragraph{Failure-rate breakdown on the $2024$--$2026$ OoS window.} For the headline $K = 18$ fits the realised breach rates (target in parentheses) are: CHMM-N $0.18\%$ ($1\%$) and $1.22\%$ ($5\%$); CHMM-t $0.18\%$ ($1\%$) and $1.40\%$ ($5\%$); CHMM-L $0.18\%$ ($1\%$) and $1.22\%$ ($5\%$). All six (model, level) cells reject Kupiec at the $5\%$ significance threshold. The rule is systematically over-conservative.

\paragraph{Mechanism.} Proposition~\ref{prop:filter_var} above gives the structural mixture-quantile inequality: when the filter places non-trivial posterior mass on at least one heavy-tailed tail-state $k^*$ and the bulk states have $F_k(q^*) < \alpha$ at the tail-state quantile $q^* = F_{k^*}^{-1}(\alpha)$, the mixture quantile satisfies $F^{-1}(\alpha) \le \min_k F_k^{-1}(\alpha)$, with strict inequality whenever the precondition holds.

\paragraph{Candidate remedies (left to future work).} Three classes of fix have been considered without resolution. (i) A concentration constraint on the filter posterior, e.g.\ floor $\pi_t(k) \ge \pi_{\min}$ on the bulk modes only, biases the mixture quantile back toward the bulk. (ii) An explicit state-conditional variance overlay that re-scales the per-state emissions by a posterior-conditional volatility factor. (iii) Rolling re-estimation of $\hat\theta$ on a moving window so the IS-fitted state library tracks the OoS regime, partially defeating the over-conservatism by widening the active state mix. None of the three is reported as a working operational rule in the present paper.

\subsection{Pre-OoS Validation $K$-Selection, Walk-Forward Summary, and $\nu_k$ Diagnostics}
\label{sec:supp_misc}

\paragraph{Pre-OoS validation $K$-selection.}
The $K = 18$ operating point in the main panel was chosen via a multi-objective criterion combining IC rank with IS and OoS distributional pass rates, which uses the same $2024$--$2026$ window later used for VaR. As a clean re-selection that decouples the two, we fit CHMM-N on $2014$-$01$-$03$ through $2021$-$12$-$31$ ($n = 2{,}013$) and score every $K$ in the sweep by the forward-algorithm held-out log-likelihood on $2022$-$01$-$03$ through $2024$-$01$-$03$ ($n = 502$); the $2024$--$2026$ window is not touched. Held-out log-lik, BIC, HQC, and CAIC all select $K^\star = 3$ (per-observation log-lik $-2.5345$ at $K = 3$ vs.\ $-2.5694$ at $K = 18$); AIC selects $K^\star = 6$; held-out two-sample KS selects $K^\star = 9$. None of the six criteria that avoid the OoS window select $K = 18$. The $K = 18$ choice is best read as a multi-metric one that weights tail fidelity (kurtosis, AD) and downstream VaR behaviour alongside likelihood; a parallel main panel at $K = 3$ would re-rank some rows in CHMM's favour on ACF-MAE and against it on tail fidelity. The $2022$--$2023$ validation slice is itself partly a structural-break period (Section~\ref{sec:rolling_origin}), so any model with substantial IS-specific structure generalises worse on it.
