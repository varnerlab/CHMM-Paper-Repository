...
\subsection{Cross-Asset Correlation Heat Maps (Pipeline~B, Companion to Table~\ref{tab:cross_asset_supp_summary})}
...
\subsection{Non-US Asset Class Extension and Per-Pair OoS Off-Diagonal Breakdown}
...
\centering
\small
\caption{\textbf{Per-pair off-diagonal correlation MAE on the seven-ticker universe} (Pipeline B, Student-t copula on CHMM-N marginals at $K = 18$, $200$ paths, $\nu^\star = 6$). For each ticker pair $(i, j)$ the table reports $|\Sigma_{\text{sim,avg}}[i,j] - \Sigma_{\text{obs}}[i,j]|$ on IS and OoS, ranked by OoS $|\Delta|$. Source: \texttt{results/track\_m12/Per\_Pair\_OffDiag\_MAE.txt}. Only the ten largest-OoS-gap pairs are shown for compactness; the full $\binom{7}{2} = 21$-pair table is in the source artefact.}
...
\subsection{Per-Asset KS Bar Chart (Pipeline~B, Sanity Check for Table~\ref{tab:cross_asset_supp_summary})}
\label{sec:cross_asset_ks_supp}

Visualisation of the per-asset KS pass rates from Table~\ref{tab:cross_asset_supp_summary}; the SIM-vs-copula gap on JPM and AAPL is the visual headline.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figs/Fig-Cross-Asset-KS-Dist.pdf}
\caption{\textbf{Per-asset IS KS pass rates after dependence injection} (Pipeline~B sanity check for Table~\ref{tab:cross_asset_supp_summary}). For each of the six tickers and each of the three dependence constructions (SIM, Gaussian copula, Student-t copula with $\nu^* = 6$), the bar reports the percentage of 200 simulated paths whose marginal distribution passes the two-sample KS test against the observed IS series at $\alpha = 0.05$. The market-factor asset SPY is approximately tied across constructions (it is the SIM engine). For non-market assets the copulas dominate because rank reordering preserves each CHMM marginal exactly; the SIM's affine propagation visibly degrades JPM and AAPL.}
