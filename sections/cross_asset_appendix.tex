...
\begin{table}[H]
\centering
\caption{SIM regression $\hat{G}_{j,t} = \hat{\alpha}_j + \hat{\beta}_j G_{\text{SPY},t} + \hat{\eta}_{j,t}$ for the five non-market assets, fitted on the $2014$-$01$-$03$ through $2024$-$01$-$03$ IS window ($T = 2{,}516$). Coefficients $\hat{\alpha}, \hat{\beta}$ and $R^2$ are reported for the factor equation used in equation~\eqref{eq:sim_sim}.}
...
\subsection{Cross-Asset Correlation Heat Maps (Pipeline~B, Companion to Table~\ref{tab:cross_asset_sim_copula})}
...
The panel ordering is SIM, Gaussian copula, Student-t copula (each reported quantitatively in Table~\ref{tab:cross_asset_sim_copula}), plus a truncated C-vine variant with SPY as root for visual comparison.
...
\subsection{Per-Asset KS Bar Chart (Pipeline~B, Sanity Check for Table~\ref{tab:cross_asset_sim_copula})}
\label{sec:cross_asset_ks_supp}

Visualisation of the per-asset KS pass rates from Table~\ref{tab:cross_asset_sim_copula}; the SIM-vs-copula gap on JPM and AAPL is the visual headline.
