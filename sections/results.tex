...
\begin{table}[t]
\centering
\caption{Descriptive statistics and stylized-fact tests for SPY daily excess growth rates. IS: 2014-01-03 through 2024-01-03 ($T = 2{,}516$); OoS: 2024-01-04 through 2026-04-20 ($T = 572$). The "annualized" Mean and Std Dev rows are moments of the annualized excess growth rate $G_t = (1/\Delta t) \ln(P_t / P_{t-1}) - r_f$ defined in equation~\eqref{eq:growth_rate} with $\Delta t = 1/252$ (drift-convention scaling), not the $\sqrt{252}$-scaled daily-return volatility. The conventional $\sqrt{252}$-annualized daily-return volatility is recovered as $\sqrt{\Delta t} \cdot \sigma(G_t)$, giving $\approx 13.8\%$ IS and $\approx 12.5\%$ OoS. JB = Jarque--Bera normality test. LB = Ljung--Box autocorrelation test at lag~20. $^*$~denotes rejection at $\alpha = 0.05$.}
...
\paragraph{Pre-OoS validation K-selection (revision response to M8).}
The $K = 18$ operating point above was chosen via a multi-objective criterion (IC rank plus IS-and-OoS distributional pass rates) that uses the same $2024$-$2026$ OoS window later used for the VaR backtest; the referee correctly flagged this as evaluation contamination. Table~\ref{tab:m8_k_selection} reports a clean re-selection that decouples the two: CHMM-N is fit on an estimation slice of $2014$-$01$-$03$ through $2021$-$12$-$31$ ($n = 2{,}013$), and every $K$ in the sweep is scored by the forward-algorithm held-out log-likelihood on an unseen validation slice of $2022$-$01$-$03$ through $2024$-$01$-$03$ ($n = 502$). The $2024$-$2026$ OoS window is not touched by this selection. Held-out log-likelihood, BIC, HQC, and CAIC all select $K^\star = 3$; AIC selects $K^\star = 6$; held-out two-sample KS selects $K^\star = 9$. None of the six criteria that avoid the OoS window select $K = 18$, and the held-out per-observation log-lik gap between $K = 3$ ($-2.5345$) and $K = 18$ ($-2.5694$) is $\sim 17$ log-lik units over the $502$-point validation slice.

The honest reading is that \emph{$K = 18$ is not the likelihood-optimal state count on SPY}; it is the operating point selected by a multi-metric criterion that weights tail fidelity (AD, kurtosis) and downstream VaR behaviour alongside likelihood, and the $2022$-$2023$ validation slice is itself partly a structural-break period (see Section~\ref{sec:rolling_and_weekly} and M4) on which any model with substantial IS-specific structure generalises worse. We therefore keep $K = 18$ as the main-panel operating point because the paper's contribution is a multi-metric evaluation (distributional, tail, temporal, VaR) rather than a likelihood-optimal single-emission fit, but we explicitly flag that a reader optimising for pure held-out log-likelihood would pick $K = 3$; the full CHMM-t / CHMM-L held-out log-lik at $K \in \{12, 15, 18, 21\}$ in Table~\ref{tab:m8_k_selection} shows this ordering is not an emission-family artefact. A parallel version of the main panel at $K = 3$ and $K = 9$ would re-rank some rows of Table~\ref{tab:model_comparison} in CHMM-$F$'s favour on ACF-MAE (which peaks at $K = 3$ per Section~\ref{sec:k_sensitivity}) and against it on tail fidelity; the operating-point choice therefore depends on which operational axis the consumer prioritises.

\begin{table}[t]
\centering
\small
\caption{\textbf{Pre-OoS validation K-selection on SPY} (revision response to M8). CHMM-N fit on the estimation slice 2014-01-03 through 2021-12-31 ($n = 2{,}013$); evaluated by held-out log-likelihood on the validation slice 2022-01-03 through 2024-01-03 ($n = 502$). The $2024$-$2026$ OoS window is not used in this selection. $K^\star = 3$ by held-out log-lik and by BIC; $K^\star = 9$ by held-out KS pass rate. The CHMM-t and CHMM-L val log-lik at $K \in \{12, 15, 18, 21\}$ sit between the CHMM-N values at the same $K$ (monotone-decreasing in $K$), so the $K^\star < 18$ ordering is not an emission-family artefact. Full CSV at \texttt{results/revision/M8\_k\_selection.csv}.}
\label{tab:m8_k_selection}
...
\midrule
$\mathbf{3}$  & $12$  & $\mathbf{-1272.3}$ & $\mathbf{-2.5345}$ & $7.0$          & $8464.8$          & $\mathbf{8532.1}$  & $\mathbf{8489.5}$ & $\mathbf{8544.1}$ \\
$6$           & $42$  & $-1273.1$          & $-2.5361$          & $7.6$          & $\mathbf{8411.2}$ & $8646.7$           & $8497.7$          & $8688.7$ \\
$9$           & $90$  & $-1280.1$          & $-2.5500$          & $\mathbf{8.0}$ & $8437.7$          & $8942.4$           & $8622.9$          & $9032.4$ \\
...
ACF-MAE of $0.0507$ was essentially on par with GARCH ($0.0484$), indicating that the CHMM reproduces volatility clustering from the transition-matrix eigenvalue spectrum alone, complementing the jump-process and conditional-variance-equation mechanisms available elsewhere in the literature.
This gives the first of two headline integrative findings in the paper: the slow-ACF behaviour that the \citet{ryden1998stylized} $K = 2$--$3$ Gaussian setup could not recover is available inside a standard HMM framework once the state resolution is moderate, complementing the HSMM and jump-augmented routes already in the literature; the second (Section~\ref{sec:conditional_var}) is a two-sided regime-conditional VaR result: the smoother-based Viterbi-decoded state-conditional quantile of CHMM-t produces an IS regime-partition goodness-of-fit diagnostic that sits below the $\chi^2_1$ Christoffersen threshold on the 2024--2026 OoS window, while the operationally correct filter-based one-step-ahead backtest fails Kupiec (over-conservative) for all three CHMM families, leaving operational real-time regime-conditional VaR as an open direction.
...
Full per-generator panels for Bin-T NJ, Block-BS, and GRU are in Appendix~\ref{sec:baselines_appendix} (Tables~\ref{tab:block_bootstrap}, \ref{tab:bin_t}, and \ref{tab:gru}).}
...
\begin{table}[t]
\centering
\caption{\textbf{Expanded GARCH-family baseline panel on SPY} (revision response to referee M7). Five additional conditional-volatility baselines fit on the same IS window used in Table~\ref{tab:model_comparison}: exponential GARCH (EGARCH(1,1)) and Glosten-Jagannathan-Runkle GARCH (GJR-GARCH(1,1)) Gaussian (asymmetric leverage), GARCH(1,1) Student-t (heavy-tailed competitor to CHMM-t), heterogeneous autoregressive realised volatility (HAR-RV) on daily squared returns (long-memory volatility), and Markov-switching GARCH at $K = 3$ (extending the two-regime MS-GARCH already in Table~\ref{tab:extended_metrics}). Numbers are re-run under the revision seed \texttt{20260422} with $N_{\text{paths}} = 1{,}000$; the panel is reported alongside, not replacing, Table~\ref{tab:model_comparison}. FIGARCH is deferred to a follow-up pass (see \texttt{revision-code-todo.md}).}
\label{tab:garch_suite}
\small
\begin{tabular}{l cc cc c c cc cc}
\toprule
& \multicolumn{2}{c}{KS (\%) $\uparrow$} & \multicolumn{2}{c}{AD (\%) $\uparrow$} & Kurt & ACF-MAE & \multicolumn{2}{c}{$\text{LR}_{\text{uc}}$} & \multicolumn{2}{c}{$\text{LR}_{\text{ind}}$} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){8-9} \cmidrule(lr){10-11}
Model & IS & OoS & IS & OoS & IS & $\downarrow$ & $1\%$ & $5\%$ & $1\%$ & $5\%$ \\
\midrule
EGARCH(1,1)        & $6.3$  & $34.9$ & $0.1$  & $2.9$  & $2.80$   & $0.0430$ & $\mathbf{0.01}$ & $\mathbf{3.03}$ & $9.15$ & $4.71$ \\
GJR-GARCH(1,1)     & $40.1$ & $57.7$ & $8.5$  & $49.5$ & $7.58$   & $0.0330$ & $\mathbf{0.27}$ & $\mathbf{0.10}$ & $9.15$ & $3.18$ \\
\textbf{GARCH-t ($\nu = 6.89$)} & $\mathbf{57.3}$ & $\mathbf{80.8}$ & $48.6$ & $\mathbf{80.4}$ & $15.13$  & $0.0316$ & $\mathbf{0.01}$ & $\mathbf{0.01}$ & $6.76$ & $2.10$ \\
HAR-RV (daily)     & $0.0$  & $0.0$  & $0.0$  & $0.0$  & $188.95$ & $0.0566$ & $5.99$ & $\mathbf{3.03}$ & $0.00$ & $4.71$ \\
MS-GARCH $K = 3$   & $36.1$ & $33.1$ & $17.9$ & $22.9$ & $4.10$   & $\mathbf{0.0284}$ & $\mathbf{0.01}$ & $\mathbf{0.82}$ & $5.73$ & $5.87$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Reading the expanded GARCH panel.}
GARCH-t with $\hat{\nu} = 6.89$ is the headline new row: it more than doubles the GARCH(1,1) Gaussian IS KS pass rate ($57.3\%$ vs.\ $25.9\%$ in Table~\ref{tab:model_comparison}), reaches the smallest OoS AD in the panel ($80.4\%$), produces a simulated IS excess kurtosis of $15.13$ close to CHMM-t's $14.57$, and passes Kupiec at both $\alpha$ levels with $\text{LR}_{\text{uc}} \approx 0.01$.
The direct comparison with CHMM-t ($K = 18$, IS KS $93.7\%$, ACF-MAE $0.0548$) is more instructive than the Gaussian-only comparison: GARCH-t slightly beats CHMM-t on ACF-MAE ($0.0316$ vs.\ $0.0548$) and matches it on kurtosis, while CHMM-t is materially ahead on KS / AD.
GJR-GARCH's asymmetry coefficient $\hat{\gamma} = 0.225$ confirms a sizeable leverage effect in SPY IS; its modestly-improved IS KS ($40.1\%$ vs.\ $25.9\%$) is consistent with that asymmetric term capturing left-tail mass that plain GARCH(1,1) misses.
MS-GARCH at $K = 3$ recovers a three-regime volatility structure ($\sigma = [0.10, 1.77, 10.04]$ for calm / medium / stress) and attains the smallest ACF-MAE in the whole panel ($0.0284$); on KS it improves on the two-regime specification ($36.1\%$ vs.\ $27.7\%$ on the same seed) but remains below CHMM-t.
HAR-RV on daily squared returns is documented in the panel as a long-memory reference but is not a competitive return simulator at the margin: its simulated excess kurtosis of $188.95$ is driven by the log-RV residual variance ($\hat\sigma_\eta = 2.449$) propagating through the exponential link, and in practice HAR-RV is a better model for realised variance than for returns themselves; this is an honest negative finding on HAR-RV-as-return-generator and does not reflect on HAR-RV-as-volatility-model.
...
We compute empirical $\text{MMD}^2$ \citep{gretton2012kernel} between $500$ observed and $500$ simulated $20$-day windows under a Gaussian radial basis function (RBF) kernel with median-heuristic bandwidth.
...
Cross-sectional correlation reproduction was acceptable on average (off-diagonal MAE $0.076$, averaged over the $15$ unique upper-triangular entries of the six-asset correlation matrix; see Appendix~\ref{sec:metrics_appendix}) but visibly worse than both copulas, consistent with SIM's rank-one decomposition discarding joint information beyond the market factor.
...
\subsection{Rolling-Origin OoS and Weekly-Frequency Robustness (Revision Response to M4)}
\label{sec:rolling_and_weekly}

In response to referee comment M4, we complement the single $2024$--$2026$ OoS window of Sections~\ref{sec:model_comparison}--\ref{sec:extended_evaluation} with (i) a rolling-origin OoS evaluation on SPY spanning $2022$--$2026$ and (ii) a weekly-frequency evaluation on the same SPY IS window. Reruns use seed \texttt{20260422} and $N_{\text{paths}} = 500$.

\paragraph{Rolling-origin OoS.}
Five rolling windows with an eight-year IS and a one-year OoS ($2022, 2023, 2024, 2025$, and $2026$-year-to-date) are reported in Table~\ref{tab:m4_rolling}. The central finding is that the single-window result in Table~\ref{tab:model_comparison} is \emph{not} representative across windows: the $2022$ OoS window (IS $2014$--$2021$) is a catastrophic failure on distributional fidelity ($0.2$--$0.4\%$ OoS KS for all three emission families) and on unconditional Kupiec calibration ($\text{LR}_{\text{uc}}^{5\%} = 31.6$--$41.4$), because the rate-hike cycle that unfolded in $2022$ is a structural break not represented in the preceding eight years of near-zero rates. The $2023$ OoS window is intermediate ($87$--$96\%$ OoS KS with $\text{LR}_{\text{uc}}^{5\%} \approx 14$, still failing Kupiec). The $2024$, $2025$, and $2026$-year-to-date windows all pass Kupiec at $5\%$ cleanly with OoS KS in the $87$--$97\%$ range, consistent with the single-window number of Table~\ref{tab:model_comparison}. This is an honest negative-plus-positive finding that directly addresses the stationarity-scope limitation already flagged in Section~\ref{sec:discussion}: the CHMM scaffold is well-calibrated when the IS window contains volatility regimes comparable to those of the OoS window, and fails predictably under structural breaks that lie outside the IS support. A rolling refit of $\hat{\mathbf{T}}$ and the emissions each quarter (Section~\ref{sec:discussion}) is the natural operational remedy.

\begin{table}[t]
\centering
\small
\caption{\textbf{Rolling-origin OoS on SPY} (revision response to M4). Five rolling windows with eight-year IS and one-year OoS; IS/OoS KS pass rate at $\alpha = 0.05$ and Kupiec $\text{LR}_{\text{uc}}$ at $\alpha \in \{0.01, 0.05\}$ on the unconditional pooled-archive VaR. \textbf{Bold} entries mark $\text{LR}_{\text{uc}}$ below the $\chi^2_1 = 3.84$ threshold. Full per-window detail is in \texttt{results/revision/M4\_rolling\_origin.csv}.}
\label{tab:m4_rolling}
\begin{tabular}{c l cc cc cc}
\toprule
 & & \multicolumn{2}{c}{KS (\%) $\uparrow$} & \multicolumn{2}{c}{$\text{LR}_{\text{uc}}^{1\%}$} & \multicolumn{2}{c}{$\text{LR}_{\text{uc}}^{5\%}$} \\
\cmidrule(lr){3-4} \cmidrule(lr){5-6} \cmidrule(lr){7-8}
OoS & Model & IS & OoS & value & br\% & value & br\% \\
\midrule
\multirow{3}{*}{$2022$} & CHMM-N & $99.0$ & $0.2$  & $7.73$ & $2.4$ & $41.4$ & $11.2$ \\
                         & CHMM-t & $98.0$ & $0.4$  & $5.50$ & $2.4$ & $33.9$ & $10.4$ \\
                         & CHMM-L & $97.0$ & $0.2$  & $3.56$ & $2.0$ & $31.6$ & $10.4$ \\
\midrule
\multirow{3}{*}{$2023$} & CHMM-N & $94.2$ & $93.8$ & $5.01$ & $2.4$ & $14.0$ & $7.6$  \\
                         & CHMM-t & $96.2$ & $93.4$ & $5.01$ & $2.4$ & $14.0$ & $7.6$  \\
                         & CHMM-L & $91.6$ & $96.0$ & $5.01$ & $2.4$ & $14.0$ & $7.6$  \\
\midrule
\multirow{3}{*}{$2024$} & CHMM-N & $93.2$ & $88.2$ & $5.05$ & $2.4$ & $\mathbf{3.05}$ & $3.2$  \\
                         & CHMM-t & $94.8$ & $87.8$ & $5.05$ & $2.4$ & $\phantom{0}4.42$ & $2.8$  \\
                         & CHMM-L & $96.0$ & $87.0$ & $5.05$ & $2.4$ & $\phantom{0}4.42$ & $2.8$  \\
\midrule
\multirow{3}{*}{$2025$} & CHMM-N & $96.0$ & $92.6$ & $\mathbf{0.10}$ & $0.8$ & $\mathbf{1.08}$ & $4.0$  \\
                         & CHMM-t & $94.8$ & $88.6$ & $\mathbf{0.10}$ & $0.8$ & $\mathbf{0.52}$ & $4.4$  \\
                         & CHMM-L & $92.8$ & $87.4$ & $\mathbf{0.10}$ & $0.8$ & $\mathbf{1.08}$ & $4.0$  \\
\midrule
\multirow{3}{*}{$2026$*} & CHMM-N & $97.2$ & $97.0$ & $\mathbf{1.47}$ & $0.0$ & $\mathbf{2.81}$ & $2.7$  \\
                         & CHMM-t & $98.2$ & $94.6$ & $\mathbf{1.47}$ & $0.0$ & $\mathbf{2.81}$ & $2.7$  \\
                         & CHMM-L & $99.4$ & $95.2$ & $\mathbf{1.47}$ & $0.0$ & $\mathbf{2.81}$ & $2.7$  \\
\bottomrule
\end{tabular}
\end{table}

The aggregate picture across the five rolling windows (CHMM-t, mean $\pm$ std): IS KS $96.4\% \pm 1.6\%$, OoS KS $72.9\% \pm 38.6\%$, unconditional $5\%$ Kupiec $\text{LR}_{\text{uc}}$ mean $13.1$ with one dominating window. Excluding the structural-break $2022$ window (which is the single most informative piece of negative evidence in the revision), the remaining four windows give CHMM-t IS KS $95.8\% \pm 1.6\%$, OoS KS $91.1\% \pm 3.5\%$, $5\%$ Kupiec $\text{LR}_{\text{uc}}^{5\%}$ mean $5.4$. The single $2024$--$2026$ OoS window reported in the main Table~\ref{tab:model_comparison} is therefore closer to the modal behaviour than the worst-case; the paper's headline fidelity claims survive the rolling-origin generalisation on four of five windows, and the $2022$ window is an explicit counter-example that the stationarity assumption needs to be relaxed for structural-break regimes.

\paragraph{Weekly-frequency.}
Table~\ref{tab:m4_weekly} reports the weekly-aggregated SPY IS evaluation ($T_{\text{weekly}} = 503$, five-day non-overlapping sum of daily log growth rates, $K \in \{6, 12\}$). All three emission families produce $98$--$99.6\%$ IS KS pass rates at both $K$, indicating that the unified CHMM scaffold transfers to a coarser sampling frequency without re-architecture; the $K = 6$ operating point is the natural choice given the smaller sample size. The kurtosis story carries over with one twist: at weekly frequency and $K = 6$, CHMM-t achieves simulated excess kurtosis of $3.82$ against observed $4.19$ (the closest match in the panel), CHMM-N and CHMM-L both undershoot ($1.98$ and $2.44$ respectively), and CHMM-t at $K = 12$ overshoots again ($7.17$, following the same daily-frequency pattern of Section~\ref{sec:extended_evaluation}). The ACF-MAE on $|r|$ at lags $1$--$50$ is $0.046$--$0.057$ across the panel, consistent with the daily ACF-MAE of $0.05$.

\begin{table}[t]
\centering
\small
\caption{\textbf{Weekly-frequency evaluation on SPY IS} (revision response to M4). Five-day non-overlapping aggregation of daily log growth rates, $T_{\text{weekly}} = 503$, $2014$--$2024$. Observed weekly excess kurtosis is $4.19$. CSV at \texttt{results/revision/M4\_weekly.csv}.}
\label{tab:m4_weekly}
\begin{tabular}{c l c c c c}
\toprule
$K$ & Model & IS KS (\%) $\uparrow$ & sim Kurt & obs Kurt & ACF-MAE $\downarrow$ \\
\midrule
\multirow{3}{*}{$6$} & CHMM-N & $98.8$ & $1.98$ & $4.19$ & $0.0459$ \\
                      & \textbf{CHMM-t} & $98.6$ & $\mathbf{3.82}$ & $4.19$ & $0.0550$ \\
                      & CHMM-L & $98.2$ & $2.44$ & $4.19$ & $0.0566$ \\
\midrule
\multirow{3}{*}{$12$} & CHMM-N & $99.2$ & $2.10$ & $4.19$ & $0.0568$ \\
...
\paragraph{Setup.}
For each of the six main-study tickers (SPY, NVDA, JNJ, JPM, AAPL, QQQ) we re-train the three CHMM families at $K = 18$ on the full IS window and simulate $n_{\text{paths}} = 500$ price paths of length $T_{\text{OoS}} = 572$ days, converting simulated returns to prices via $\widetilde P_t = \widetilde P_{t-1} \cdot \exp((\widetilde G_t + r_f)\,\Delta t)$ with $\Delta t = 1/252$, $r_f = 0$, and $S_0$ the last IS volume-weighted average price (VWAP).
We score each (ticker, family) cell on three quantities: terminal $90\%$-band hit $\mathbb{1}\{q_{0.05} \le P^{\text{obs}}_T \le q_{0.95}\}$; horizon-average band coverage $\overline{C}_{0.90} = T_{\text{OoS}}^{-1} \sum_t \mathbb{1}\{q_{0.05}(t) \le P^{\text{obs}}_t \le q_{0.95}(t)\}$; and the bias-corrected sample continuous ranked probability score (CRPS) of \citet{gneiting2007strictly}, normalised by $S_0$ for cross-ticker comparability.
