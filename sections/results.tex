...
\subsection{Rolling-Origin OoS and Weekly-Frequency Robustness}
\label{sec:rolling_and_weekly}

We complement the single $2024$--$2026$ OoS window of Sections~\ref{sec:model_comparison}--\ref{sec:extended_evaluation} with (i) a rolling-origin OoS evaluation on SPY spanning $2022$--$2026$ and (ii) a weekly-frequency evaluation on the same SPY IS window. Reruns use seed \texttt{20260422} and $N_{\text{paths}} = 500$.
...
\centering
\small
\caption{\textbf{Rolling-origin OoS on SPY}. Five rolling windows with eight-year IS and one-year OoS; IS/OoS KS pass rate at $\alpha = 0.05$ and Kupiec $\text{LR}_{\text{uc}}$ at $\alpha \in \{0.01, 0.05\}$ on the unconditional pooled-archive VaR. \textbf{Bold} entries mark $\text{LR}_{\text{uc}}$ below the $\chi^2_1 = 3.84$ threshold. Full per-window detail is in \texttt{results/robustness/rolling\_origin\_oos.csv}.}
...
\centering
\small
\caption{\textbf{Weekly-frequency evaluation on SPY IS}. Five-day non-overlapping aggregation of daily log growth rates, $T_{\text{weekly}} = 503$, $2014$--$2024$. Observed weekly excess kurtosis is $4.19$. CSV at \texttt{results/robustness/weekly\_frequency.csv}.}
\label{tab:m4_weekly}
