...
\begin{table}[t]
\centering
\caption{\textbf{Expanded GARCH-family baseline panel on SPY} (revision response to referee M7). Five additional conditional-volatility baselines fit on the same IS window used in Table~\ref{tab:model_comparison}: EGARCH(1,1) and GJR-GARCH(1,1) Gaussian (asymmetric leverage), GARCH(1,1) Student-t (heavy-tailed competitor to CHMM-t), HAR-RV on daily squared returns (long-memory volatility), and Markov-switching GARCH at $K = 3$ (extending the two-regime MS-GARCH already in Table~\ref{tab:extended_metrics}). Numbers are re-run under the revision seed \texttt{20260422} with $N_{\text{paths}} = 1{,}000$; the panel is reported alongside, not replacing, Table~\ref{tab:model_comparison}. FIGARCH is deferred to a follow-up pass (see \texttt{revision-code-todo.md}).}
