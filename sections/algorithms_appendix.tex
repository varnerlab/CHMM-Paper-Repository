...
This two-block ECM preserves the monotonicity of the standard EM guarantee under Assumption~\ref{ass:compact}, with the search invariant $Q_k(\nu^{(n+1)}) \ge Q_k(\nu^{(n)})$ at every iteration; Proposition~\ref{prop:ecm_monotone} in Section~\ref{sec:theory} states the guarantee in full.
