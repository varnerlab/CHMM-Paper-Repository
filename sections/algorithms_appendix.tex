...
\begin{equation}
    ...
    \label{eq:mstep}
\end{equation}
The convergence criterion is $|\mathcal{L}^{(n)} - \mathcal{L}^{(n-1)}| < 10^{-4}$ with $\mathcal{L}^{(n)} = \text{logsumexp}_k \log \alpha_T^{(n)}(k)$, and simulation draws $S_1$ from the stationary distribution of $\hat{\mathbf T}$ and then iterates $G_t \sim \mathcal{N}(\mu_{S_t}, \sigma_{S_t}^2)$, $S_{t+1} \sim \text{Categorical}(\mathbf{T}_{S_t, \cdot})$ with no jump process.
...
\begin{equation}
    ...
    \label{eq:nu_pen}
\end{equation}
maximised by the same golden-section search ($\lambda = 0$ recovers the unpenalised \citet{peel2000robust, liu1995ml} ECM; the rate sweep and its effect on simulated IS kurtosis are in the main text Discussion).
...
\begin{equation}
    \mu_k^{(n+1)} \;=\; \arg\min_{\mu \in I_k} \;\sum_t \gamma_t(k)\, \big|O_t - \mu\big|^{p_k^{(n)}},
...
\begin{equation}
    ...
    \label{eq:ged_p}
\end{equation}
again by a $40$-iteration golden-section search over $[p_{\min}, p_{\max}] = [0.5, 3.0]$. The scale update is the unique zero of $\partial Q_k/\partial \alpha$, so CM-2 is exact maximisation, while CM-1 and CM-3 are bracketed maximisations. Each CM step is therefore a partial maximisation, and the standard ECM monotonicity result~\citep{mengrubin1993ecm} applies on the compact bracket; the guarantee is stated in full in Proposition~\ref{prop:ecm_monotone}. At the boundary cases, $p_k = 2$ recovers Gaussian (with $\sigma = \alpha/\sqrt{2}$) and $p_k = 1$ recovers Laplace (with $b = \alpha$).
...
\FOR{$n = 1$ to $\texttt{max\_iter}$}
    ...
    \STATE \qquad $\mu_k \leftarrow \arg\min_{\mu \in I_k} \sum_t \gamma_t(k)\, |O_t - \mu|^{p_k}$ \COMMENT{40-iter golden-section search on $I_k = [\min_t O_t, \max_t O_t]$.}
...
The simulated IS mixture kurtosis of $17.44$ was driven by the posterior mass routed to those two very heavy-tailed states, not by a systemic collapse to the lower bracket.
