...
\begin{equation}
    \begin{aligned}
    \mu_k^{(n+1)} &\;=\; \operatorname{GoldenSearchMin}_{\mu \in I_k^{(n)}}
        \sum_t \gamma_t(k)\, \big|O_t - \mu\big|^{p_k^{(n)}},
        \qquad I_k^{(n)} = [\ell_k^{(n)}, u_k^{(n)}],\\[-2pt]
    \ell_k^{(n)} &\;=\; \min\!\left(\mu_k^{(n)} - 5\alpha_k^{(n)},\, \min_t O_t - 10^{-6}\right),\\[-2pt]
    u_k^{(n)} &\;=\; \max\!\left(\mu_k^{(n)} + 5\alpha_k^{(n)},\, \max_t O_t + 10^{-6}\right).
    \end{aligned}
    \label{eq:ged_mu}
\end{equation}
by a $40$-iteration golden-section search over the current location-scale bracket widened to include the observed data range; CM-2 updates the scale in closed form given $(\mu_k^{(n+1)}, p_k^{(n)})$,
\begin{equation}
...
\FOR{$n = 1$ to $\texttt{max\_iter} + 1$}
    ...
    \STATE \qquad $\mu_k \leftarrow \operatorname{GoldenSearchMin}_{\mu \in I_k} \sum_t \gamma_t(k)\, |O_t - \mu|^{p_k}$ \COMMENT{40 iterations; $I_k$ is the current $(\mu_k,\alpha_k)$ bracket widened to include $[\min_t O_t,\max_t O_t]$.}
