...
\label{sec:supp_chmm_architecture}
...
\label{sec:supp_pipeline_summary}

For the single-asset analysis, observed returns $G_t$ are fit by Baum-Welch CHMM (body operating point $K^\star = 3$ with $K = 18$ as the kurtosis-fidelity sensitivity reference; the appendix per-ticker rollup of Table~\ref{tab:sector_panel} and the multi-asset marginals of Table~\ref{tab:cross_asset} use $K = 18$), then $1{,}000$ paths are simulated and per-asset metrics computed (Table~\ref{tab:model_comparison}). For the multi-asset analysis we reuse these single-asset $K = 18$ per-asset fits as marginals, draw a copula sample $\mathbf U \sim \mathcal C(\Sigma)$ from the fitted Student-$t$ copula on ranks, rank-reorder the per-asset single-asset paths to match $\mathbf U$, and report cross-asset metrics (Table~\ref{tab:cross_asset}). The single-asset CHMM scaffold is shared across both; the CHMM training and simulation steps are Algorithms~\ref{alg:chmm_em} and~\ref{alg:chmm_simulate}, the cross-asset rank-reordering step is Algorithm~\ref{alg:copula_sim}, and the architecture diagram for the single-asset scaffold is Figure~\ref{fig:chmm_architecture}.
