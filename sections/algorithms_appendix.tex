...
Algorithm~\ref{alg:chmm_em} is the unified EM procedure shared across all four emission families: a shared scaffold (quantile-based initialisation, log-space forward-backward, transition update) plus four family-specific branches at lines~\ref{line:init}, \ref{line:emit}, and \ref{line:mstep}. Algorithm~\ref{alg:chmm_simulate} is the simulator. Algorithm~\ref{alg:copula_sim} is the cross-asset rank-reordering simulator (Pipeline~B). Viterbi and SIM-resampling pseudocode is in \texttt{CHMM-Model/src/Compute.jl} and \texttt{src/CrossAsset.jl} respectively; both are textbook constructions and we omit them from the appendix.
...
% Viterbi pseudocode omitted (textbook construction; reference implementation in CHMM-Model/src/Compute.jl).
...
% reference implementation in CHMM-Model/src/CrossAsset.jl).
...
% CHMM architecture diagram has been promoted to body Figure~\ref{fig:chmm_architecture}
% in sections/model.tex (arXiv-prep pass).
