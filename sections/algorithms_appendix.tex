...
Algorithm~\ref{alg:chmm_em} is the unified EM procedure shared across all four emission families: a shared scaffold (quantile-based initialisation, log-space forward-backward, transition update) plus four family-specific branches at lines~\ref{line:init}, \ref{line:emit}, and \ref{line:mstep}. Algorithm~\ref{alg:chmm_simulate} is the simulator. Algorithm~\ref{alg:copula_sim} is the cross-asset rank-reordering simulator (Pipeline~B). Viterbi and SIM-resampling pseudocode is provided in the companion code repository; both are textbook constructions and we omit them from the appendix.
...
Pipeline~A: observed returns $G_t \to$ fit CHMM (Baum-Welch, body operating point $K^\star = 3$ with $K = 18$ as the kurtosis-fidelity sensitivity reference; the appendix per-ticker rollup of Table~\ref{tab:sector_panel} and the Pipeline-B marginals of Table~\ref{tab:cross_asset} use $K = 18$) $\to$ simulate $1{,}000$ paths $\to$ per-asset metrics (Table~\ref{tab:model_comparison}). Pipeline~B reuses the Pipeline-A $K = 18$ per-asset fits as marginals, draws a copula sample $\mathbf U \sim \mathcal C(\Sigma)$ from the fitted Student-$t$ copula on ranks, rank-reorders the per-asset Pipeline-A paths to match $\mathbf U$, and reports cross-asset metrics (Table~\ref{tab:cross_asset}). The single-asset scaffold is shared across both pipelines; the CHMM training and simulation steps are Algorithms~\ref{alg:chmm_em} and~\ref{alg:chmm_simulate}, the cross-asset rank-reordering step is Algorithm~\ref{alg:copula_sim}, and the architecture diagram for the single-asset scaffold is Figure~\ref{fig:chmm_architecture}.
