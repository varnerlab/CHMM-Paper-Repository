...
For Laplace emissions $b_k(x) = \tfrac{1}{2 \beta_k} \exp(-|x - \mu_k|/\beta_k)$ (with the per-state Laplace scale renamed from the conventional $b_k$ to $\beta_k$ to avoid the symbol collision with the emission-density notation $b_k(\cdot;\boldsymbol\theta_k)$ used in Section~\ref{sec:chmm_def}) the weighted-MLE estimators are available in closed form:
\begin{equation}
...
\subsubsection{Pipeline Schematic}
\label{sec:supp_pipeline_schematic}
...
\subsubsection{CHMM Architecture and Training Loop}
...
\node[emit, minimum width=28mm, right=2mm of eT] (eL)
    {\textbf{CHMM-L} \\ $b_k = \mathrm{Laplace}(\mu_k, \beta_k)$};
