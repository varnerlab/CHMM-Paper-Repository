...
Unlike the discrete-state framework of \citet{alswaidan2026hybrid}, which requires Laplace quantile binning and a Poisson jump-duration mechanism to achieve volatility clustering, we show that the continuous model at moderate state resolution ($K = 6$ to $13$) reproduces all three stylized facts (heavy tails, negligible linear autocorrelation, and persistent volatility clustering) without jump augmentation, thereby eliminating two tunable hyperparameters ($\epsilon$, $\lambda$) and the discretization step entirely.

We benchmark the CHMM against four alternative generators on ten years of daily S\&P~500 ETF (SPY) data: bootstrap resampling, Gaussian and Laplace i.i.d.\ generators, and GARCH(1,1).
...
\input{sections/introduction_v3}

\section{Related Work}
\label{sec:related}
\input{sections/related_v3}

\section{Methods}
\label{sec:methods}
\input{sections/methods_v3}
...
\input{sections/supplemental_v3}
