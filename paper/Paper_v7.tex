...
Applying the same EM pipeline to the Chicago Board Options Exchange (CBOE) Volatility Index (VIX) log-return series confirms that the framework also covers the volatility measures most commonly used as risk inputs; option pricing is explicitly out of scope.
The seven-metric panel supplies the fidelity dimension of the ACM Journal of Data and Information Quality (JDIQ) fidelity/utility/privacy triad, the VaR/ES back-test supplies a utility consumer, and privacy is out of scope by construction because SPY and VIX data are public.
