...
We further extend the synthetic-data framework beyond equities to the CBOE Volatility Index (VIX) and the VXX short-term volatility ETN, showing that the same Baum-Welch pipeline reproduces the distinct statistical properties of volatility measures (positive skewness, pronounced mean reversion, and heavier-than-equity excess kurtosis) and passes the same seven-metric distributional battery used on equity returns.
The result is a single synthetic-data engine that covers both the equity cross-section and the volatility measures most commonly used as risk inputs, in the same spirit as our earlier discrete HMM paper \citep{alswaidan2026hybrid}.
Option pricing and implied-volatility-surface modeling are explicitly out of scope for this paper.
...
\label{sec:related}
\input{sections/related_v4}

\section{Methods}
\label{sec:methods}
\input{sections/methods_v4}
...
\input{sections/supplemental_v4}
