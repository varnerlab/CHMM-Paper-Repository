...
\node[font=\small\bfseries, anchor=west, below=6mm of paData.south west] (pbTitle) {Pipeline~B (cross-asset, Section~\ref{sec:cross_asset}):};
\node[data, below=2mm of pbTitle.south west, anchor=north west] (pbData) {$d$ tickers\\ returns $G^{(i)}_t$};
\node[model, right=of pbData] (pbMarg) {Per-asset\\ CHMM fits\\ (marginals)};
...
% side labels clarifying the decomposition
\node[lbl, anchor=west] at ($(pbMarg.north)+(0,1.4mm)$) {marginals};
\node[lbl, anchor=center] at ($(pbCop.north)+(0,1.4mm)$) {dependence};
...
The \textit{discrete HMM} of \citet{alswaidan2026hybrid} is reproduced in both no-jump (NJ) and Poisson-jump (WJ) variants with the authoritative hyperparameters of the prior-paper final SPY run: returns are partitioned into $K_{\text{disc}} = 90$ quantile bins, transitions are estimated by frequency counting, and simulation returns bin centroids; the WJ variant additionally triggers, with probability $\epsilon = 5 \times 10^{-5}$, a forced residence in tail states for Poisson$(\lambda = 67)$ steps, matching the values used in the prior-paper SPY simulation notebook.
The \textit{Bin-T NJ} variant replaces those bin centroids with bin-conditional Student-t emissions to isolate the quantization-per-se effect from the emission-family effect; for the Bin-T NJ variant only we retain a coarser $K_{\text{disc}} = 13$ bin count so that within-bin tail mass is estimable from a reasonable number of observations per bin.
