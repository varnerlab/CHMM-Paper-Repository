...
\begin{tikzpicture}[
    ...
    box/.style={draw, rectangle, rounded corners=1.5pt, align=center,
                minimum height=8mm, minimum width=22mm, inner sep=1.5mm,
                fill=black!3},
...
\node[font=\small\bfseries, anchor=west, below=11mm of paData.south west] (pbTitle) {Pipeline~B (cross-asset, Section~\ref{sec:cross_asset}):};
\node[data, below=4mm of pbTitle.south west, anchor=north west] (pbData) {$d$ tickers\\ returns $G^{(i)}_t$};
...
% ---- EM training loop (right of emission panel) ----
\node[block, right=12mm of eL] (estep)
    {E-step\\ forward--backward\\ $\gamma_t(k),\ \xi_t(i,j)$};
\node[block, above=4mm of estep] (mstep)
...
\node[lbl, left=1mm of estep, rotate=90, anchor=south, yshift=1mm] {};
