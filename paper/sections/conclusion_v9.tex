...
A volatility-index robustness check (Block~D, Section~\ref{sec:results_vix}) applied the identical three-family CHMM scaffold to daily $\log(\text{VIX})$ levels on the same 2014--2023 training window.
The fitted models reproduce the stationarity ($100\%$ ADF rejection of a unit root on $200$ simulated paths), slow-decay ACF of log-levels (ACF-MAE $0.22$--$0.26$), and positively skewed heavy-tailed first differences (Hill $\hat\alpha \approx 2.6$ vs.\ $3.21$ observed) that the log-mean-reverting VIX literature \cite{papanicolaou2014regime, kaeck2013stochastic, dotsis2007empirical} identifies as the canonical stylized-fact panel for a volatility-index generator, without any VIX-specific code change.
This extends the digital-twin framing from the return-series level to the algorithmic-scaffold level.

Option pricing, implied-volatility-surface calibration, joint equity--VIX coupling, and leverage-effect modelling are explicitly out of scope for this paper.
...
\begin{enumerate}
    ...
    \item \textit{Joint equity--volatility twin and leverage effect.}
    Block~D validates the CHMM scaffold as a standalone volatility-index generator with the two marginals fitted independently.
    A natural extension is a bivariate digital twin that composes the equity and VIX marginals through a copula conditioned on the latent equity regime, capturing the leverage effect \cite{papanicolaou2014regime} and enabling multi-asset scenario generation that includes realized-volatility observables as first-class outputs.
    VIX options pricing and VVIX-style volatility-of-volatility modelling \cite{huang2023fear} are orthogonal extensions that we do not attempt here.
\end{enumerate}
...
C.J.\ proposed the volatility-index generalization in Block~D, contributed to methodology review and validation, and co-developed the VIX data-alignment and diagnostic pipeline.
