...
The direct head-to-head against the discrete framework of \citet{alswaidan2026hybrid}, reproduced as the Discrete NJ and Discrete WJ rows of Table~\ref{tab:model_comparison}, reveals a 95.9-percentage-point swing in IS KS pass rate (0\% for either discrete variant $\to$ 95.9\% for the CHMM).
...
The seven-model benchmark confirmed that no single generator dominates all metrics, but the CHMM at $K = 18$ achieves the most balanced performance: 95.9--96.0\% IS KS across the three emission families (vs.\ 11.7\% GARCH and 0.0\% for either discrete variant), ACF-MAE of 0.0499 under CHMM-N (vs.\ 0.0470 GARCH), Wasserstein-1 of 0.091 under CHMM-t (the smallest of any parametric model), and 100\% quantile coverage.
Cross-asset univariate generalization to NVDA (99.1\% IS KS), JNJ (99.4\%), JPM (98.0\%), AAPL (99.4\%), and QQQ (94.5\%) confirmed adaptability to diverse equity risk profiles, and out-of-sample evaluation on held-out 2025 data showed robust generalization on SPY/NVDA/AAPL/QQQ but a sector-specific $\sim$30-percentage-point OoS KS drop on defensives (JNJ, JPM) that is invariant to the choice of $K$ and therefore attributable to the stationarity assumption rather than model selection.
...
All revision experiments reported in Section~\ref{sec:results} and in the appendix use a deterministic seed (\texttt{V7\_SEED = 20260420}) applied globally at the top of the analysis script \texttt{run\_v7\_revisions.jl}, with per-experiment sub-seeds derived additively to keep each block independently reproducible.
Software environment: Julia \(\geq\) 1.12 with the exact package versions pinned in \texttt{Manifest.toml} at the repository root; running \texttt{Pkg.instantiate()} inside the project directory reproduces the dependency graph, after which \texttt{include("run\_v7\_revisions.jl")} regenerates every table and figure that is new in v7.
