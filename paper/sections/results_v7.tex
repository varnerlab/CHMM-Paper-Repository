...
\subsection{Out-of-Sample Equity Price Simulation}
...
\begin{subfigure}[t]{0.32\textwidth}
    ...
    \caption{CHMM-L.}
\end{subfigure}
\caption{SPY out-of-sample price fan across the three CHMM emission families (500 paths, $T_{\text{OoS}} = 572$ trading days, $K = 18$). Blue band: $5\%$--$95\%$ quantile envelope of simulated paths; darker band: $25\%$--$75\%$ interquartile range; blue line: simulated median; red line: observed OoS price track. $S_0 = 469.90$; $P^{\text{obs}}_T = 710.13$.}
...
\begin{subfigure}[t]{0.32\textwidth}
    ...
    \caption{CHMM-L.}
\end{subfigure}
\caption{NVDA out-of-sample price fan across the three CHMM emission families (500 paths, $T_{\text{OoS}} = 572$ trading days, $K = 18$). Same plotting conventions as Figure~\ref{fig:price_fan_spy}. $S_0 = 47.67$; $P^{\text{obs}}_T = 198.07$. The realized 2024--2025 NVDA rally is extreme relative to the prior-decade IS distribution; the observed path exits the interquartile band but remains inside the $5\%$--$95\%$ envelope under all three emission families.}
...
\end{subfigure}
\caption{Terminal-price distributions under CHMM-N for the two in-body tickers (500 paths, $T_{\text{OoS}} = 572$). Red vertical line: observed OoS terminal $P^{\text{obs}}_T$; blue dashed line: simulated-path median. The right-skew is characteristic of log-returns aggregated to prices; the SPY terminal is centered close to the simulated median, while the NVDA terminal sits in the upper right quadrant of the simulated distribution. Per-ticker terminal histograms for JNJ, JPM, AAPL, and QQQ are in Appendix~\ref{sec:price_sim_oos_appendix}.}
...
\subsection{VIX-Conditioned Equity Price Forecasting}
\label{sec:price_sim_vix}

Section~\ref{sec:price_sim_oos} treated each ticker in isolation: the equity CHMM was simulated forward autonomously with no external covariate.
A natural extension, motivated by the joint equity-volatility literature \citep{whaley2009vix, rossi2006volatility}, is to condition the equity regime at each step on the contemporaneous state of a dedicated VIX CHMM, so that episodes of elevated volatility mapped by the VIX filter propagate through to equity emissions.
This subsection implements that conditioning via a lightweight two-model coupling and evaluates it on the same price-forecasting battery as Section~\ref{sec:price_sim_oos}, with no further hyperparameter tuning.

\paragraph{Setup.}
We train a Gaussian CHMM-N on the CBOE VIX daily log-return series at $K = 18$ and, for each main-study ticker, train the three equity CHMM families (N, t, L) at $K = 18$ on the full IS window.
The forward filter posterior $\pi^V_t(s) = P(S^V_t = s \mid y^V_{1:t})$ is computed in log space with $\texttt{forward\_filter}$ (using the same log-space $\alpha$-recursion as in Appendix~\ref{sec:algorithms}), and the posterior at the last IS day $\pi^V_{T^V_{\text{IS}}}$ is used to seed the simulated VIX regime at the start of the OoS window.
We then Viterbi-decode both the VIX CHMM and each equity CHMM on the shared IS window and estimate the row-stochastic coupling matrix
\begin{equation}
C_i[s^V, s^E] \;=\; \widehat P\!\big(S^{E_i}_t = s^E \,\big|\, S^V_{t - h} = s^V\big)
\label{eq:coupling_C}
\end{equation}
with lag $h = 1$ and Laplace smoothing at $\alpha = 1$ per cell to avoid zero-probability transitions under finite samples, a standard regularization in discrete conditional-probability estimation \citep{rabiner1986introduction}.
The lag $h = 1$ enforces leading-indicator semantics: equity regime at day $t$ is predicted from the VIX regime at day $t - 1$, avoiding the contemporaneous circularity that would otherwise pollute the forecast.
The joint sampler, implemented as \texttt{simulate\_prices\_vix\_conditioned}, steps as follows at each $t$: (i) draw $s^V_t$ from the VIX transition row $A^V[s^V_{t-1}, :]$ (or, at $t = 1$, from $\pi^V_{T^V_{\text{IS}}}$); (ii) draw $s^E_t$ from the coupling row $C_i[s^V_t, :]$; (iii) emit a return $\widetilde G_t$ from the equity CHMM's per-state emission, and (iv) roll the price forward through Equation~\eqref{eq:price_sim}.
Unlike the Gaussian/Student-t copulas of Section~\ref{sec:cross_asset}, which preserve each equity marginal exactly via rank reordering but treat the equity CHMMs as a priori independent \citep{sklar1959fonctions, demarta2005tcopula}, the coupling sampler preserves each CHMM's state-space structure while letting an exogenous volatility driver steer the regime selection.

\begin{figure}[t]
\centering
\begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-Coupling-SPY.pdf}
    \caption{SPY coupling $C_i[s^V, s^E]$.}
\end{subfigure}
\begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-Coupling-NVDA.pdf}
    \caption{NVDA coupling $C_i[s^V, s^E]$.}
\end{subfigure}
\caption{Empirical coupling matrices $C_i[s^V, s^E]$ from Equation~\eqref{eq:coupling_C} with lag $h = 1$ and Laplace smoothing $\alpha = 1$. Rows: VIX regime (low row index = low-volatility state); columns: equity regime. For both tickers, several VIX rows concentrate mass on a narrow band of equity regimes, which is the coupling signal exploited by the joint sampler; rows near the top of the matrix (calm VIX regimes) concentrate on low-variance equity states, while the bottom rows (crash-state VIX regimes) shift mass toward high-variance equity states. Coupling heatmaps for JNJ, JPM, AAPL, and QQQ are in Appendix~\ref{sec:price_sim_vix_appendix}.}
\label{fig:coupling_heatmaps}
\end{figure}

\paragraph{Full-horizon metrics.}
Table~\ref{tab:price_sim_vix_metrics} reports the four-metric battery (terminal $90\%$-band hit, horizon-wide $\overline{C}_{0.90}$, $\text{MAPE}_{0.50}$, and scale-free $\overline{\text{CRPS}}/S_0$) under the VIX-conditioned sampler, and also the row-averaged coupling entropy $\overline{H}(C_i)$ in nats as a summary of how informative the coupling is; the unconditional upper bound is $\log K = \log 18 \approx 2.89$, so entropies closer to $2.89$ indicate a coupling that is nearly uniform and therefore offers little predictive value, while entropies well below $2.89$ indicate informative coupling.

\begin{table}[t]
\centering
\caption{VIX-conditioned price-forecast metrics on the OoS window ($K = 18$, $500$ paths, $T_{\text{OoS}} = 572$, $h = 1$, seed = 20260421). $\textsc{Hit}_{90}$: terminal $90\%$-band indicator. $\overline{C}_{0.90}$: horizon-wide $90\%$-band coverage. $\text{MAPE}_{0.50}$: median-path MAPE. $\overline{\text{CRPS}}/S_0$: scale-free CRPS. $\overline{H}(C_i)$: row-averaged entropy of the coupling matrix in nats (ceiling $\log 18 \approx 2.89$).}
\label{tab:price_sim_vix_metrics}
\small
\begin{tabular}{ll c c r r r}
\toprule
Ticker & Family & $\textsc{Hit}_{90}$ & $\overline{C}_{0.90}$ & $\text{MAPE}_{0.50}$ & $\overline{\text{CRPS}}/S_0$ & $\overline{H}(C_i)$ \\
\midrule
\multirow{3}{*}{SPY}  & CHMM-N & \checkmark & 1.000 & 0.097 & 0.0784 & 2.51 \\
                      & CHMM-t & \checkmark & 0.949 & 0.157 & 0.1332 & 2.62 \\
                      & CHMM-L & \checkmark & 0.969 & 0.171 & 0.1467 & 2.53 \\
\midrule
\multirow{3}{*}{NVDA} & CHMM-N & \checkmark & 0.437 & 0.454 & 0.9575 & 2.61 \\
                      & CHMM-t & \checkmark & 0.545 & 0.380 & 0.7416 & 2.81 \\
                      & CHMM-L & \checkmark & 0.523 & 0.439 & 0.8974 & 2.80 \\
\midrule
\multirow{3}{*}{JNJ}  & CHMM-N & \checkmark & 0.935 & 0.095 & 0.0781 & 2.77 \\
                      & CHMM-t & \checkmark & 0.995 & 0.097 & 0.0742 & 2.81 \\
                      & CHMM-L & \checkmark & 0.995 & 0.098 & 0.0747 & 2.80 \\
\midrule
\multirow{3}{*}{JPM}  & CHMM-N & \checkmark & 0.937 & 0.235 & 0.2388 & 2.58 \\
                      & CHMM-t & \checkmark & 0.811 & 0.287 & 0.3015 & 2.74 \\
                      & CHMM-L & \checkmark & 0.948 & 0.250 & 0.2600 & 2.79 \\
\midrule
\multirow{3}{*}{AAPL} & CHMM-N & \checkmark & 1.000 & 0.128 & 0.1097 & 2.72 \\
                      & CHMM-t & \checkmark & 1.000 & 0.110 & 0.1088 & 2.79 \\
                      & CHMM-L & \checkmark & 1.000 & 0.103 & 0.1060 & 2.79 \\
\midrule
\multirow{3}{*}{QQQ}  & CHMM-N & \checkmark & 0.993 & 0.160 & 0.1383 & 2.52 \\
                      & CHMM-t & \checkmark & 0.997 & 0.136 & 0.1127 & 2.52 \\
                      & CHMM-L & \checkmark & 0.995 & 0.119 & 0.0988 & 2.76 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-SPY-PriceFan-N.pdf}
    \caption{CHMM-N.}
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-SPY-PriceFan-t.pdf}
    \caption{CHMM-t.}
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-SPY-PriceFan-L.pdf}
    \caption{CHMM-L.}
\end{subfigure}
\caption{SPY VIX-conditioned price fan. $500$ paths, $T_{\text{OoS}} = 572$, coupling lag $h = 1$. Plotting conventions as in Figure~\ref{fig:price_fan_spy}, with a green colourway to distinguish from the unconditional sampler.}
\label{fig:price_fan_spy_vix}
\end{figure}

\begin{figure}[t]
\centering
\begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-NVDA-PriceFan-N.pdf}
    \caption{CHMM-N.}
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-NVDA-PriceFan-t.pdf}
    \caption{CHMM-t.}
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-NVDA-PriceFan-L.pdf}
    \caption{CHMM-L.}
\end{subfigure}
\caption{NVDA VIX-conditioned price fan. Same conventions as Figure~\ref{fig:price_fan_spy_vix}. The NVDA rally of 2024--2025 is a level effect unconnected to the VIX state process, so conditioning on VIX cannot help here and mildly narrows the fan; the observed path exits the simulated envelope more often than under the unconditional sampler.}
\label{fig:price_fan_nvda_vix}
\end{figure}

\begin{figure}[t]
\centering
\begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-SPY-TerminalDist-N.pdf}
    \caption{SPY terminal (CHMM-N).}
\end{subfigure}
\begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[width=\textwidth]{sections/figs/Fig-VIX-NVDA-TerminalDist-N.pdf}
    \caption{NVDA terminal (CHMM-N).}
\end{subfigure}
\caption{VIX-conditioned terminal-price distributions for the two in-body tickers (CHMM-N, $500$ paths). Red line: observed OoS terminal; green dashed line: simulated-path median. Per-ticker terminal histograms for JNJ, JPM, AAPL, and QQQ under the VIX-conditioned sampler are in Appendix~\ref{sec:price_sim_vix_appendix}.}
\label{fig:price_terminal_hists_vix}
\end{figure}

\paragraph{Interpretation.}
Coupling signal is modest-but-present: the row-averaged $\overline{H}(C_i)$ sits between $2.51$ and $2.81$ across tickers, with a theoretical ceiling of $\log 18 \approx 2.89$; SPY and QQQ (the two market-like tickers) show the strongest coupling ($\overline{H} \approx 2.51$), consistent with the market-factor role of VIX, whereas NVDA, AAPL, JNJ, and JPM hover closer to uniform.
Pairwise against Section~\ref{sec:price_sim_oos}, VIX-conditioning produces small-to-moderate changes in the forecast metrics that are not uniformly better: SPY's CHMM-N CRPS$/S_0$ improves from $0.0835$ to $0.0784$, AAPL's CHMM-L improves from $0.1021$ to $0.1060$ (a slight regression), JPM's CHMM-N degrades from $0.1881$ to $0.2388$, and NVDA's CRPS worsens across the board (CHMM-N $0.6424 \to 0.9575$), because the NVDA OoS rally is a regime-drift phenomenon rather than a VIX-driven state change and conditioning on a narrower volatility regime tightens the fan away from the realized path.
The aggregate picture is that VIX-conditioning helps precisely the tickers for which VIX is an economically meaningful covariate, and neither helps nor hurts for tickers whose OoS behaviour is disconnected from broad-market volatility; these results are consistent with the probabilistic-forecasting calibration-sharpness framework of \citet{gneiting2014probabilistic} and argue for adaptive coupling (e.g., regime-specific or factorial-HMM style) as the natural next step, in line with the stretch goal discussed in the supplementary implementation plan.
The underlying public API (\texttt{forward\_filter}, \texttt{fit\_coupling}, \texttt{simulate\_prices\_vix\_conditioned}) extends cleanly to options and to the cross-asset dependence machinery of Section~\ref{sec:cross_asset} without further changes.
Per-ticker VIX-conditioned fans, terminal histograms, and coupling heatmaps for JNJ, JPM, AAPL, and QQQ are in Appendix~\ref{sec:price_sim_vix_appendix}.
