...
\begin{subfigure}[t]{0.32\textwidth}
    ...
    \caption{JPM terminal.}
\end{subfigure}
\caption{Terminal-price distributions under CHMM-N for three representative tickers (500 paths, $T_{\text{OoS}} = 572$). Red vertical line: observed OoS terminal $P^{\text{obs}}_T$; blue dashed line: simulated-path median. The right-skew is characteristic of log-returns aggregated to prices; SPY and JPM terminals are centered close to the simulated median, while the NVDA terminal is in the upper right quadrant of the simulated distribution.}
...
Per-ticker fans and terminal histograms for the remaining tickers, as well as the raw path-metric CSV, are available in the supplementary result archive at \texttt{results/equity\_price\_sim/}.
