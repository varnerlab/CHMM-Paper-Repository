...
Cross-asset generalization is assessed on three additional equities spanning distinct risk profiles: NVDA (NVIDIA Corporation; high-beta technology), JNJ (Johnson \& Johnson; low-beta healthcare), and JPM (JPMorgan Chase \& Co.; moderate-beta financials).
Each asset is fitted independently using the same training/test split.

To extend the framework beyond equity returns, we additionally apply it to the CBOE Volatility Index (VIX), which measures the 30-day implied volatility of S\&P~500 options and is widely regarded as the market's ``fear gauge.''
...
VIX log returns exhibit statistical properties that differ markedly from equity returns---positive skewness (reflecting asymmetric volatility spikes), pronounced mean reversion, and extreme excess kurtosis---making it a challenging test case that probes the generality of the CHMM framework.
...
We define the continuous hidden Markov model (CHMM) as the tuple $\mathcal{M} = (\mathcal{S}, \mathcal{O}, \mathbf{T}, \mathbf{B}, \boldsymbol{\pi})$ \citep{rabiner1986introduction}, where:
\begin{itemize}
...
The Baum-Welch algorithm \citep{baum1970maximization} is the standard expectation-maximization (EM) procedure for maximum likelihood estimation of HMM parameters.
...
Define the forward variable $\alpha_t(k) = \Prob(O_1, O_2, \ldots, O_t, S_t = k \mid \mathcal{M})$ and the backward variable $\beta_t(k) = \Prob(O_{t+1}, O_{t+2}, \ldots, O_T \mid S_t = k, \mathcal{M})$.
The recursions are:
\begin{align}
...
\begin{equation}
    ...
    \label{eq:logsumexp}
\end{equation}
All forward-backward computations are performed entirely in log-space to prevent the floating-point underflow that occurs when multiplying many small probabilities for long observation sequences \citep{bilmes1998gentle}.
...
The EM algorithm is well known to be sensitive to initial parameter values, with poor starting configurations leading to degenerate local optima where all states collapse to the global mean \citep{rabiner1986introduction}.
...
As the primary temporal benchmark, we fit a GARCH(1,1) model \citep{bollerslev1986generalized}:
\begin{align}
...
We evaluate synthetic data quality using seven complementary metrics, following the framework of \citet{stenger2024thinking} and \citet{alswaidan2026hybrid}.
For each model, 1,000 independent paths of length $T$ are simulated.
Each metric is computed per path and then aggregated.

\begin{enumerate}
    \item \textbf{KS pass rate (\%).}
    The proportion of 1,000 simulated paths for which the two-sample Kolmogorov--Smirnov test \citep{kolmogorov1933, smirnov1948} fails to reject the null hypothesis of distributional equivalence with the reference data at significance level $\alpha = 0.05$.
    ...
    The mean absolute difference of sorted empirical quantiles, equivalent to the $L^1$ distance between the empirical CDFs \citep{glasserman2003monte}:
    \begin{equation}
...
\subsection{VIX Extension and Option Pricing Pathway}
\label{sec:vix_methods}

To demonstrate the generality of the framework beyond equity returns, we train the CHMM on VIX daily log returns computed as:
\begin{equation}
    R_t^{\text{VIX}} = \ln\!\left(\frac{\text{VIX}_t}{\text{VIX}_{t-1}}\right)
    \label{eq:vix_returns}
\end{equation}

The trained CHMM provides a regime decomposition of VIX dynamics via the Viterbi algorithm, which decodes the most likely hidden state sequence $S_{1:T}^{*} = \arg\max_{S_{1:T}} \Prob(S_{1:T} \mid O_{1:T}, \mathcal{M})$.
The median VIX level within each decoded regime defines a volatility map $\bar{V}_k = \text{median}(\text{VIX}_t : S_t^* = k)$ that links each HMM state to a characteristic market volatility level.

This regime decomposition provides a natural bridge to option pricing.
The volatility map $\bar{V}_k$ parameterizes a regime-switching geometric Brownian motion (GBM):
\begin{equation}
    dP_t = \mu\, P_t\, dt + \frac{\bar{V}_{S_t}}{100}\, P_t\, dW_t
    \label{eq:regime_gbm}
\end{equation}
where $S_t$ follows the CHMM's learned Markov dynamics.
Monte Carlo simulation of \eqref{eq:regime_gbm} produces option prices under the CHMM-implied regime structure, which we compare against the Heston stochastic volatility benchmark \citep{heston1993closed} to assess whether the discrete regime approximation captures the key features of implied volatility surfaces.
