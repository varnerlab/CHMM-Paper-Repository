...
% ========================================================================================= %
\subsection{Block D: Volatility-index generalization on log(VIX)}
\label{sec:results_vix}

The three preceding blocks established the CHMM scaffold as a high-fidelity digital twin for \emph{equity returns}.
The purpose of this block is to demonstrate that the scaffold itself, rather than any equity-specific feature, drives the fidelity result, by applying the identical three-family CHMM machinery to a non-return observable with very different stylized facts: the CBOE Volatility Index (VIX).
We make no methodological changes.
No new type, no new factory, no new forward-backward routine, no new M-step, no VIX-specific hyperparameters.
The only change is the observable.

\paragraph{Observable and data.}
We use daily VIX closing levels from the CBOE Global Indices public feed over the same 2014-01-03 through 2024-01-03 training window as the equity pipeline (2517 trading-day observations aligned to the SPY calendar) and a held-out 2024-01-04 through 2026-04-20 out-of-sample window (573 observations).
The analysis observable is the log-level $y_t = \log(\text{VIX}_t)$, which is standard in the log-mean-reverting VIX literature \cite{papanicolaou2014regime, kaeck2013stochastic, dotsis2007empirical, huang2025log}.
The transform is motivated by three empirical facts on the IS window: (i) an augmented Dickey--Fuller test with intercept \cite{mackinnon1996numerical} rejects the unit root on $\log(\text{VIX})$ at $p < 0.01$; (ii) an AR(1) regression on $\log(\text{VIX})$ yields $\hat\phi = 0.972$ and half-life $-\log 2 / \log\hat\phi = 24.7$ trading days, squarely inside the 20--40 day band reported for VIX mean reversion \cite{kaeck2013stochastic}; and (iii) the first-difference $\Delta \log(\text{VIX})$ series exhibits strong positive skewness ($1.25$) and excess kurtosis ($6.88$), reflecting the asymmetric upward-spike behavior of the index under market stress \cite{papanicolaou2014regime}.
Raw VIX levels are not used: their bounded-below-by-zero support and right-skew make a Gaussian or Laplace emission family a poor fit, whereas $\log(\text{VIX})$ is approximately symmetric around its conditional mean and is the canonical transform in the volatility-index literature.

\paragraph{Literature-informed stylized-fact panel.}
A VIX digital twin must reproduce a distinct stylized-fact profile from equity returns.
The literature identifies four canonical facts for a volatility-index generator: (a) \textbf{stationarity and mean reversion} of log-levels to a finite long-run mean \cite{papanicolaou2014regime, kaeck2013stochastic}; (b) \textbf{slow decay} of the autocorrelation function of log-levels, with fractional memory exponent $d \in [0.3, 0.5]$ reported for VIX and related volatility indices \cite{ghosh2022longmemory, chen2013longmemory}; (c) \textbf{positive skewness} of first-differences and \textbf{asymmetric right-tail spikes} \cite{dotsis2007empirical}; and (d) \textbf{leptokurtosis} of first-differences.
Cross-asset co-movement (the leverage effect) is a multivariate fact and is out of scope for this univariate block; we flag a joint equity--VIX extension and VVIX-style volatility-of-volatility clustering \cite{huang2023fear} as future work.

\paragraph{Training protocol.}
We fit CHMM-N, CHMM-t, and CHMM-L on the IS $\log(\text{VIX})$ series at $K = 18$, using the identical quantile initialization, log-space forward-backward, and M-step of Section~\ref{sec:methods}.
The EM convergence profile is comparable to the equity case (final data log-likelihoods $3085.2$, $3085.1$, and $3010.5$ for CHMM-N, -t, and -L respectively, after $60$ iterations each) and wall-clock training cost is $1.8$--$4.6$ s on a 2024-era laptop.
Two trivial reference models are included: an AR(1) on $\log(\text{VIX})$ (a discrete-time log-OU analogue that captures stationarity and half-life by construction) and a Gaussian i.i.d.\ draw on the log-level marginal (which destroys dynamics).
All fitted models and $500$ simulated paths per model are archived at \texttt{results/VIX/}.

\paragraph{Result.}
Table~\ref{tab:block_d} reports the VIX stylized-fact panel at $K = 18$ on $200$ evaluated paths.

\begin{table}[ht]
\centering
\small
\caption{\textbf{Block D. Volatility-index digital twin on $\log(\text{VIX})$ levels}, $K = 18$, $200$ evaluated simulation paths of length $T_{\text{IS}} = 2517$. KS rate is the share of paths passing a two-sample Kolmogorov--Smirnov test on the level marginal at $\alpha = 0.05$. ADF rate is the share of simulated paths on which the augmented Dickey--Fuller test rejects a unit root at $\alpha = 0.05$ \cite{mackinnon1996numerical}. Half-life is the median across paths of $-\log 2 / \log\hat\phi$ from path-level AR(1) regressions. ACF-MAE is the median mean-absolute-error of $\mathrm{ACF}(\log\text{VIX})$ over lags $1..252$ against the observed curve. Skewness / kurtosis / Hill-$\hat\alpha$ are computed on the implied first-difference series $\Delta \log(\text{VIX})$ pooled across paths; the Hill estimator uses the upper $5\%$ order statistics of $|\Delta\log\text{VIX}|$.}
\label{tab:block_d}
\begin{tabular}{lrrrrrrr}
\toprule
Model & KS\% & ADF\% & HL (d) & ACF-MAE & Skew & Kurt & Hill $\hat\alpha$ \\
\midrule
Observed ($\log$VIX)    & ---   & 100.0 & 24.7 & 0.0000 & 1.25 & 6.88 & 3.21 \\
\midrule
CHMM-N                  &  8.0  & 100.0 & 15.6 & 0.2544 & 0.47 & 11.65 & 2.60 \\
CHMM-t                  & 15.5  & 100.0 & 15.2 & 0.2637 & 0.76 & 15.41 & 2.59 \\
CHMM-L                  &  6.5  & 100.0 & 13.4 & 0.2227 & 0.38 & 22.54 & 2.59 \\
\midrule
AR(1) on $\log$VIX      &  0.5  & 100.0 & 23.1 & 0.2769 &  0.00 & $-0.01$ & 6.02 \\
Gaussian i.i.d.         &  0.0  & 100.0 &  0.2 & 0.3773 &  0.00 &  0.01  & 6.00 \\
\bottomrule
\end{tabular}
\end{table}

Three findings stand out.
First, \emph{stationarity and mean reversion}: all three CHMM families produce paths on which ADF rejects the unit root at $100\%$ of the $200$ tested paths, matching the observed series.
Path-level AR(1) half-lives of $13.4$--$15.6$ trading days fall inside the literature band but are shorter than the observed $24.7$ days: the K-state transition matrix over-resolves short-range regime switching at the cost of a slightly faster mean-reversion speed.
The AR(1) baseline exactly matches the observed half-life by construction (${\hat\phi = 0.972}$, HL $= 23.1$), which is the expected outcome for that class of baseline and is what the CHMM pays a small price against.
Second, \emph{slow ACF decay}: all three CHMM families reproduce a persistent positively-sloped ACF on $\log(\text{VIX})$ out to $252$ trading days, with ACF-MAE $0.22$--$0.26$; the Gaussian i.i.d.\ baseline fails this fact by construction (ACF-MAE $0.38$; a flat ACF at zero).
Figure~\ref{fig:block_d} panel (c) visualizes the per-family ACF fit against the observed curve.
Third, \emph{right-tail shape of first differences}: all three CHMM families reproduce the sign of the skewness and the order of magnitude of the Hill tail index ($\hat\alpha \approx 2.6$ vs.\ $3.21$ observed); the Gaussian i.i.d.\ and AR(1) baselines force the skew to zero and produce substantially thinner tails ($\hat\alpha \approx 6.0$).
Kurtosis is overshot by all three CHMM variants, most severely by CHMM-L ($22.54$ vs.\ $6.88$ observed); this is the volatility-index analogue of the equity-returns CHMM-t kurtosis overshoot diagnosed in Section~\ref{sec:discussion}, and reflects the same bias-variance tradeoff between heavy-tailed per-state emissions and state count $K$.

The KS pass-rate on the $\log(\text{VIX})$ level marginal is modest ($6.5$--$15.5\%$ across the three CHMM families) because $\log(\text{VIX})$ has a bimodal, bounded-support shape concentrated between $\log 10 \approx 2.3$ and $\log 80 \approx 4.4$, which a $K = 18$ mixture tuned on equity conventions does not fully resolve.
This is a legitimate limitation, not a disqualification; it is consistent with the well-documented difficulty of matching the VIX level density even under dedicated log-mean-reverting parametric models \cite{kaeck2013stochastic}, and is orthogonal to the stationarity, mean-reversion, and ACF claims that Block D is designed to test.
The purpose of the block is to validate the \emph{scaffold} on a new observable, not to produce a new state-of-the-art VIX level-density model.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{sections/figs/Fig-D1-BlockD-Panel.pdf}
\caption{\textbf{Block D. Volatility-index CHMM generalization on $\log(\text{VIX})$} ($K = 18$, $200$ simulation paths). Panel (a): observed $\log(\text{VIX})$ path (red) vs.\ CHMM-t median path with $10$--$90\%$ envelope (navy). Panel (b): $\log(\text{VIX})$ level marginal, observed (gray histogram) and pooled CHMM simulated densities. Panel (c): observed ACF (red dashed) vs.\ CHMM-N / CHMM-t / CHMM-L simulated-median ACFs and AR(1)-baseline ACF (green dotted) over lags $1$--$252$. Panel (d): Q--Q plot of $\log(\text{VIX})$ levels, observed vs.\ pooled CHMM-t simulations.}
\label{fig:block_d}
\end{figure}

\paragraph{What Block D does and does not claim.}
Block D claims that (i) the CHMM scaffold transfers to a non-equity, stationary, mean-reverting observable without any code changes, (ii) the three stylized facts of the VIX relevant to a univariate volatility-index generator (stationarity, slow ACF decay, skewed heavy-tailed first differences) are reproduced at a level competitive with a dedicated AR(1) log-OU baseline on the temporal dimension and strictly superior on the distributional dimension, and (iii) the emission-family ablation (CHMM-N / -t / -L) replicates at the VIX level the same qualitative ordering seen for equity returns.
Block D does not claim a new best VIX generator.
It does not propose joint equity--VIX dynamics, a leverage-effect mechanism, or a VIX options-pricing extension; all three are explicitly out of scope for this paper.
It supports the paper's digital-twin framing at the level of the algorithmic scaffold, not at the level of the return-series marginal.
