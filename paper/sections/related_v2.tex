...
This section reviews the key developments that motivate the present work, with particular attention to the Gaussian HMM limitation identified by \citet{ryden1998stylized} that our results challenge.

\subsection{Stylized Facts of Asset Returns}

The Black--Scholes--Merton framework \citep{black1973pricing} assumes that log-returns are independently and identically distributed (i.i.d.) Gaussian random variables.
...
Stochastic volatility (SV) models treat the log-variance as a latent process.
The \citet{heston1993closed} model specifies the variance as a mean-reverting square-root diffusion:
\begin{equation}
    dv_t = \kappa(\theta - v_t)\,dt + \sigma_v \sqrt{v_t}\,dW_t^v
    \label{eq:heston_lit}
\end{equation}
correlated with the price diffusion via $\text{Corr}(dW_t^S, dW_t^v) = \rho$.
SV models produce realistic volatility dynamics and closed-form option prices, but parameter estimation requires computationally intensive methods (e.g., particle filtering, MCMC).
The Heston model features prominently in our VIX extension, where we use CHMM-decoded regimes to parameterize the mean-reversion target $\theta$.

Jump-diffusion models \citep{merton1976option, kou2002jump} augment the geometric Brownian motion with Poisson-driven jumps in the price level, producing heavy-tailed marginal distributions.
...
HMMs have since been applied to financial time series for regime identification \citep{kim1999has}, volatility estimation \citep{rossi2006volatility}, and trading strategy design \citep{nguyen2018hidden}.
...
\begin{enumerate}
    \item HMMs produce heavy-tailed marginal distributions through the Gaussian mixture mechanism, even though each component is thin-tailed.
    \item HMMs produce negligible linear autocorrelation in returns, consistent with the observed stylized fact.
    \item HMMs \emph{fail} to reproduce the slow decay of the ACF of absolute returns (volatility clustering).
\end{enumerate}
...
HSMMs replace the geometric sojourn-time distribution with flexible parametric alternatives (e.g., negative binomial, log-series), allowing arbitrarily slow ACF decay.
...
(i)~continuous Gaussian emissions replace Laplace quantile binning, removing discretization artifacts;
(ii)~the Baum-Welch algorithm jointly learns emissions and transitions, replacing frequency counting;
(iii)~no jump mechanism is needed at moderate $K$, removing two hyperparameters and the grid search entirely.
