...
A continuous Gaussian HMM (CHMM) trained via the Baum-Welch algorithm \cite{baum1970maximization, rabiner1986introduction} with quantile-based initialization reproduces all three canonical stylized facts at moderate state resolution \emph{without} requiring jump augmentation, discretization, or separate emission fitting.
We sweep the state resolution over $K \in \{3, 6, 9, 12, 15, 18, 21\}$, report the full sensitivity in the appendix, and select $K = 18$ as the single main-body benchmark on the basis of a multi-metric score: $K = 18$ is the only value that wins (or ties) on all four distributional pass-rate metrics (IS/OoS~KS and IS/OoS~AD) while also attaining the smallest Wasserstein-1 error and the largest simulated kurtosis across the sweep.
...
\begin{enumerate}
    ...
    At $K = 18$, the resulting CHMM achieves 96.8\% IS and 95.0\% OoS KS pass rates, AD pass rates of 90.1\% IS and 87.2\% OoS, and ACF-MAE of 0.051 on absolute returns, all without any jump mechanism, and eliminating the two hyperparameters ($\epsilon$, $\lambda$) and the discretization step.
    ...
    \item \textbf{Seven-model benchmark including the discrete HMM of the prior paper.}
    We evaluate the CHMM against six alternatives on identical metrics and identical data: bootstrap resampling, Gaussian and Laplace i.i.d.\ generators, the \emph{discrete} HMM of \citet{alswaidan2026hybrid} in both no-jump (NJ) and Poisson-jump (WJ, $\epsilon = 0.01$, $\lambda = 3$) variants, and GARCH(1,1).
    Seven complementary metrics (KS and Anderson--Darling pass rates, excess kurtosis, ACF-MAE, Wasserstein-1 and Hellinger distances, and quantile coverage) are reported on 1{,}000 simulated paths in both in-sample (2014--2024) and out-of-sample (2025) windows.
    The direct discrete-vs-continuous comparison isolates the contribution of our methodological changes relative to the prior paper.

    \item \textbf{State resolution analysis with principled model selection.}
    We systematically vary $K \in \{3, 6, 9, 12, 15, 18, 21\}$ and report all seven metrics per $K$.
    Distributional fidelity improves with $K$ and peaks at $K = 18$ across IS and OoS KS and AD simultaneously; ACF-MAE is remarkably stable across all $K$ (0.046--0.051) and is lowest at $K = 3$, crystallizing a distributional--temporal tradeoff that we make explicit.
    ...
    On six representative assets, copulas substantially outperform the SIM factor baseline on per-asset in-sample KS pass rate (median 98.0\% versus 83.8\%) and on the off-diagonal correlation MAE (0.027 for Student-t, 0.031 for Gaussian, 0.078 for SIM), with the Student-t copula profile MLE selecting $\nu^* = 6$ degrees of freedom.

    \item \textbf{Unified synthetic-data generation for equities and volatility measures.}
    We confirm robustness across NVDA, JNJ, JPM, AAPL, and QQQ, and extend the same Baum-Welch pipeline to CBOE VIX and VXX log returns, demonstrating that the CHMM captures the distinct statistical properties of volatility measures (positive skewness, pronounced mean reversion, heavier tails) using the same seven-metric evaluation.
    The result is a single synthetic-data engine spanning the equity cross-section and the volatility measures most commonly used as risk inputs, matching the scope of our prior discrete HMM paper~\citep{alswaidan2026hybrid}.
    Option pricing and implied-volatility-surface calibration are explicitly out of scope for this paper and are not revisited in any section.
\end{enumerate}
...
Section~\ref{sec:results} presents the empirical results: descriptive statistics, seven-model comparison at $K = 18$, a summary state resolution sensitivity analysis, out-of-sample evaluation, cross-asset SIM and copula dependence, and the VIX/VXX extension.
