The results demonstrate that a continuous Gaussian HMM trained via Baum-Welch at a single operating point ($K = 18$) achieves strong distributional fidelity (96.8\% IS KS, 95.0\% OoS KS, 100\% quantile coverage) while retaining competitive temporal fidelity (ACF-MAE of 0.051), without requiring jump augmentation.
The direct comparison against the discrete HMM of \citet{alswaidan2026hybrid}, reproduced in both no-jump and Poisson-jump variants within our pipeline, shows a two-orders-of-magnitude swing in IS KS pass rate (0\% $\to$ 96.8\%) attributable purely to the move from quantized to continuous emissions.
...
GARCH(1,1) achieves the best ACF-MAE (0.047) among parametric models but catastrophically fails KS (11.5\%), while the bootstrap achieves perfect KS (100.0\%) but the worst ACF-MAE (0.062).

The CHMM at $K = 18$ occupies a distinctive position in this tradeoff: it achieves IS KS of 96.8\% and IS AD of 90.1\% --- close to the non-parametric bootstrap --- while simultaneously matching GARCH's ACF-MAE to within 0.003.
It also achieves the smallest Wasserstein-1 (0.099) of any parametric model, competitive Hellinger (0.077), and 100\% quantile coverage.
...
Despite the CHMM's strong performance on distributional tests, a systematic kurtosis gap persists: the observed excess kurtosis is 7.71, while the CHMM produces 3.70--5.11 depending on $K$, peaking at 5.11 at $K = 18$.
...
The kurtosis gap explains why the AD pass rate (90.1\%) is consistently lower than the KS pass rate (96.8\%): the Anderson--Darling test assigns greater weight to the tails, where the Gaussian mixture's lighter-than-observed tails produce the largest discrepancies.
...
The M-step update for Student-$t$ emissions requires iterative estimation of the degrees-of-freedom parameter $\nu_k$ per state, adding computational cost but no conceptual difficulty, and is the natural immediate extension of this work.
...
The cross-asset univariate results confirm that the CHMM generalizes to equities with diverse risk profiles (NVDA: 99.0\% IS KS; AAPL: 99.6\%; QQQ: 95.4\%).
At $K = 18$ the OoS degradation concentrates on JNJ (71.9\%) and JPM (74.5\%), reflecting the stationarity assumption: the CHMM's transition matrix and emission parameters are fixed at their IS-estimated values, so regime shifts between the training and test periods, which may arise from sector-specific events or macroeconomic shocks, are not captured.
...
\begin{enumerate}
    ...
    The Gaussian assumption systematically underestimates excess kurtosis (5.1 simulated vs.\ 7.7 observed at $K = 18$), limiting the model's ability to reproduce extreme tail events.
    ...
    \item \textit{No formal model selection beyond the multi-metric score.}
    We select $K = 18$ as the operating point on the basis of simultaneously winning all four distributional pass-rate metrics plus $W_1$ and simulated kurtosis, and we report the full sweep in the appendix, but we do not apply information-theoretic criteria (AIC, BIC) directly.
    A principled BIC/held-out-log-likelihood-based selection would confirm (or refine) the multi-metric choice.
\end{enumerate}
