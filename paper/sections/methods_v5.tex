...
\subsection{Continuous Gaussian Hidden Markov Model}
\label{sec:chmm_def}

We model the continuous excess growth rate $G_t$ as a CHMM $\mathcal{M} = (\mathcal{S}, \mathbf{T}, \mathbf{B}, \boldsymbol{\pi})$ with $K$ latent states, Gaussian emissions $G_t \mid S_t = k \sim \mathcal{N}(\mu_k, \sigma_k^2)$, transition matrix $\mathbf{T} \in \mathbb{R}^{K \times K}$, and initial distribution $\boldsymbol{\pi}$ \cite{rabiner1986introduction}.
The stationary marginal is the $K$-component Gaussian mixture
\begin{equation}
    f(x) = \sum_{k=1}^{K} \bar{\pi}_k\, \mathcal{N}(x; \mu_k, \sigma_k^2),
    \qquad \bar{\boldsymbol{\pi}} = \bar{\boldsymbol{\pi}} \mathbf{T},
    \label{eq:mixture}
\end{equation}
and supplies heavy tails through the spread of component means and variances without requiring heavy-tailed emissions.
Unlike the discrete framework of \citet{alswaidan2026hybrid}, which bins observations and estimates $\mathbf{T}$ by frequency counting, we learn $\mathbf{T}$, $\{(\mu_k, \sigma_k)\}$, and $\boldsymbol{\pi}$ jointly from continuous observations by the Baum-Welch algorithm.

\paragraph{Baum-Welch training.}
The Baum-Welch (EM) algorithm \cite{baum1970maximization} alternates forward-backward inference with maximum-likelihood re-estimation.
All forward-backward computations are carried out in log space with a stable \texttt{logsumexp} reduction \cite{bilmes1998gentle} to prevent floating-point underflow at $T \approx 10^{3}$.
EM requires a non-degenerate initialization; we sort the $T$ observations in ascending order, partition them into $K$ equal-sized chunks, and seed each state's $(\mu_k^{(0)}, \sigma_k^{(0)})$ from the corresponding chunk, initializing $T_{ij}^{(0)} = \pi_k^{(0)} = 1/K$.
Iteration proceeds to a tolerance $|\mathcal{L}^{(n)} - \mathcal{L}^{(n-1)}| < 10^{-4}$ with $\texttt{max\_iter} = 60$.
All models in our sweep converged within 20--40 iterations.
Appendix~\ref{sec:algorithms} gives the full forward-backward recursion, the posterior quantities $\gamma_t(k)$ and $\xi_t(i, j)$, the closed-form M-step updates for $(\boldsymbol{\pi}, \mu_k, \sigma_k, \mathbf{T})$, and the simulation routine.

\paragraph{State-count selection.}
We sweep $K \in \{3, 6, 9, 12, 15, 18, 21\}$ and report per-$K$ fidelity on the full seven-metric battery below.
We additionally compute the four information criteria used for HMM state selection in \citet{nguyen2018hidden},
\begin{equation}
...
\end{equation}
where $p = K^2 + 2K - 1$ is the number of free parameters and $\mathcal{L}$ is the converged log-likelihood.
The final operating point is the $K$ that minimizes the average information-criterion rank and simultaneously wins or ties every in-sample and out-of-sample distributional pass-rate metric.
