...
Block~D (Section~\ref{sec:results_vix}) adds a volatility-index robustness check on $\log(\text{VIX})$: the identical CHMM scaffold, trained without any VIX-specific code change, reproduces the stationarity, slow-decay ACF, and skewed heavy-tailed first-difference profile that the log-mean-reverting VIX literature \cite{papanicolaou2014regime, kaeck2013stochastic, dotsis2007empirical} identifies as the canonical stylized-fact panel for a volatility-index generator.
The implication for the thesis is that the high-fidelity result is an algorithmic-scaffold property of the CHMM, not an equity-return-series property; the same code that trains a return-series digital twin also trains a volatility-index digital twin at the cost of one extra $O(KT)$ EM pass.
