...
At this state resolution a bin-centroid discrete HMM is \emph{not} a straw man on coarse distributional pass rates; a coarser $K_{\text{disc}} = 13$ discretization does open a visible KS/AD gap against the CHMM family, but that gap reflects under-resolution of the bin grid rather than a property of the discrete-state framework itself.
...
The twelve-model comparison of Table~\ref{tab:model_comparison} reveals that no single model dominates all of the reported metrics, confirming the temporal-distributional tradeoff identified in the discrete framework of \citet{alswaidan2026hybrid}.
...
The Student-t variant produces an in-sample mixture kurtosis of $14.57$ against an observed $7.68$: a $\sim 90\%$ overshoot.
