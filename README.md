...
|-- sections/                    # One .tex per section, content-named (no version suffix)
|   |-- introduction.tex
|   |-- related_work.tex
|   |-- model.tex
|   |-- estimation.tex
...
[`CHMM-Model`](https://github.com/altashly1/CHMM-Model); PDFs in
`figs/` are copied from that repo's `figs/` after each rebuild.
...
Target output: `paper.pdf` with zero undefined citations or references.
...
`CHMM-paper/figs/` before rebuilding the manuscript.
